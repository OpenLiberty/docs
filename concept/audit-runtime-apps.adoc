...
The Audit (`audit-1.0`) feature enables the capability to provide audit support for the server runtime environment and applications. When you add the Audit feature, audit data is provided in an on-premise audit log and also is available in the cloud and viewable in dashboards.
...
* Confirms the effectiveness and integrity of the existing configuration
* Identifies areas where improvement to the configuration might be needed
