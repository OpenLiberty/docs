...
The Audit (`audit-1.0`) feature enables support for providing audit support for the server runtime environment and applications. When you add the Audit feature, audit data is provided in an on-premise audit log, as well being available in the cloud and viewable in dashboards.

== Why auditing the server runtime environment and applications is important

TODO: Give context here about why it's important. What would motivate someone to enable this? Who (regulatory authorities? who else?) might require this information? What other benefits are there?

== Auditing server with a Liberty runtime
