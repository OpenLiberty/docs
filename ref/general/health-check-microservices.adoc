...
- `/health`: aggregates the responses from `/health/live` and `/health/ready`. This endpoint corresponds with the deprecated `@Health` annotation and is only available to provide compatibility with MicroProfile 1.0. Developers who are using MicroProfile 2.0 or higher should use the `/health/ready` or `/health/live` endpoints instead.
