...
For most applications that run on Open Liberty, you do not need to tune the size of the thread pool. However, in some cases, you might need to manually adjust settings for the `coreThreads` and `maxThreads` attributes.
