...
It is also possible to avoid defining roles altogether if you specify role names that match the group names in the user registry exactly. In this case, the server automatically maps the role to the group with a matching name. In this configuration, you do not specify an `<application-bnd>` element in the `server.xml` file. 
