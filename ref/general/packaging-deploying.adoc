...
You can change the default HTTP port of the server to the port where the application is accessed by replacing the `httpPort` attribute value in the `server.xml` file.
