...
Maven and Gradle are build automation tool that make managing your builds and dependencies easier.
...
With link:https://maven.apache.org/[Maven], you use a `pom.xml` file to define information about your project, and Maven downloads and installs all dependencies for you.
...
To deploy and run your application from a WAR file, first you must configure Open Liberty by defining the application with the `webApplication` element in the `server.xml` file.
...
You can change the default HTTP port of the server to the port where the application will be accessed by replacing the `httpPort` attribute value in the `server.xml` file.
