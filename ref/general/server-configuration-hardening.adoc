...
It's also a best practice to ensure that the Open Liberty server configuration file system, which is the path where the `WLP_USER_DIR` environment variable points, is only writable to authorized server administrators.
...
For example, when all 3 bits are on for an attribute, this is represented as a `7`, which indicates read, write, and execute access.
...
Generally, the last digit in the file permissions should end in `0` because others shouldn't have access to protected information.
