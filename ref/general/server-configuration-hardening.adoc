...
* Add the `webAppSecurity ssoRequiresSSL=’true’` config to the `server.xml` file.
* Add the `webAppSecurity httpOnlyCookies=’true’` config to the `server.xml` file.
* Set the `httpOptions removeServerHeader="true"` config in the `server.xml` file.
* Set the `webContainer disableXPoweredBy="true"` config in the `server.xml` file.
...
Protecting these points of attack hardens your server configuration for use in production, but there are other security vulnerabilities to consider before your container and application are production-ready.
