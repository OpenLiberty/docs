...
:page-type: general
= Development mode for deploying, testing, and debugging applications
...
To run Open Liberty in dev mode, link:https://github.com/OpenLiberty/ci.maven#configuration[enable the Open Liberty Maven plug-in] or link:https://github.com/OpenLiberty/ci.gradle#usage[the Open Liberty Gradle plug-in] and run one of the following commands:
...
- Try out dev mode for yourself with link:https://github.com/OpenLiberty/demo-devmode[the demo-devmode sample project]. This demo is available for both Maven and Gradle users. Maven users can also try out dev mode in the guide to link:/guides/getting-started.html[Packaging and deploying applications].
