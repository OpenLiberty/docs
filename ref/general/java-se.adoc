...
Open Liberty requires a Java SE runtime. A Java SE runtime, also known as a Java Runtime Environment or a Java Developer Kit (JDK), provides the tools to develop, run, and monitor your Java application. Always run Open Liberty on the most recent release of Java that is available. Open Liberty does not support any version of Java that is no longer supported by Oracle. Open Liberty runs on any of the Java runtimes listed in the following sections.
