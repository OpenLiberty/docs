...
Hardening is a general term that is used to describe the measures that are taken to enhance your system security against potential threats and risks in production. It is important to harden your Open Liberty server to reduce vulnerabilities and prevent against different types of intrusions that can occur. Different types of security intrusions can threaten different parts of your system, and there are best practices that you can employ to make each part less vulnerable. Security intrusions and vulnerabilities can be broken down into different groups:
...
* *Keep Open Liberty up to date*: To reduce operating system vulnerabilities, it's important to keep your version of Open Liberty up to date. Open Liberty fix packs are compatible with later versions, and the zero migration policy means that you can move to a newer fix pack without changing anything in your server configuration. Ensuring that you have a recent version of Open Liberty positions you for link:/docs/ref/general/#security-vulnerabilities.html[critical security fixes].
...
* *Enable the `transportSecurity` feature*: The feature:transportSecurity-1.0[] feature enables the feature:ssl-1.0[] feature to support SSL connections and adds the ability to configure outbound transports. In addition to enabling `transportSecurity-1.0`, disable non-SSL ports by setting them to `-1` in the `server.xml` file, and use only certificates that are signed by a certificate authority for public-facing production servers.
...
* *Understand roles and priorities*: Roles and mappings are crucial for securely configuring your application. Incorrect role mapping can give inadvertent levels of access to users, which can result in some users with access to sensitive resources. You define roles by using annotations and the `web.xml` file, and you bind these roles to users or groups in the `ibm-application-bnd.xm` and `server.xml` files. For more information about roles and mapping them to the appropriate users or groups, go to <link pending>.
