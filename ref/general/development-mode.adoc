...
- Try out dev mode for yourself with link:https://github.com/OpenLiberty/demo-devmode[the demo-devmode sample project]. This demo is available for both Maven and Gradle users. Maven users can also try out dev mode in the guide to link:/guides/getting-started.html[Getting started with Open Liberty].
