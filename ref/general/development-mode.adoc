...
To start dev mode with Maven, enable the Open Liberty Maven plug-in and run the `mvn liberty:dev` command.

To start dev mode with Gradle, enable the Open Liberty Gradle plug-in and run the `gradle libertyDev` command.
...
- Addition of new dependencies to your `pom.xml` file or `build.gradle` file
...
To use dev mode with Maven, link:https://github.com/OpenLiberty/ci.maven#configuration[enable the Open Liberty Maven plugin] and run the `mvn liberty:dev` command

=== Dev mode with Gradle

To use dev mode with Gradle, link:https://github.com/OpenLiberty/ci.gradle#usage[enable the Open Liberty Gradle plug-in] and run the `gradle libertyDev` command.
