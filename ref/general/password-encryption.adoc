...
In Open Liberty, the encryption key that's used for decrypting can be overridden from the default value by setting the `wlp.password.encryption.key` property.
