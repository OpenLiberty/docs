...
Microservices are designed to be deployed in multiple environments. Externalizing the configuration with MicroProfile Config helps microservices to run in different environments without a change in code.
...
MicroProfile Config API separates the configuration from code. With MicroProfile Config, you can inject external configuration into services in the containers without repackaging them.
