...
The MP-JWT specification defines how a JWT interacts with Jave EE container security so that the JWT is easily used with JAX-RS applications. For more information, see the https://ftp.fau.de/eclipse/microprofile/microprofile-jwt-auth-1.1/microprofile-jwt-auth-spec.pdf[Eclipse MicroProfile Interoperable JWT RBAC specification].
