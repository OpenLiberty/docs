...
First, create a Kerberos service principal name and keytab file and enable them for SPNEGO authentication.
...
+
----
ktpass -in myOtherKrb5.keytab -out krb5.keytab -princ HTTP/myLibertyMachine.example.com@MYDOMAIN.EXAMPLE.COM -mapUser myLibertyMachine_http -mapOp set -pass security -crypto RC4-HMAC-NT -ptype KRB5_NT_PRINCIPALCopy
----
+
...
=== Enable the Kerberos keytab and configuration files and SPNEGO web authentication
...
+
----
"HTTP/" + java.net.InetAddress.getLocalHost().getCanonicalHostName();Copy
----
+
