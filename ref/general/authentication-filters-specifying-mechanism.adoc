...
For example, you can configure an authentication filter on the Social Media Login feature to specify a particular type of social media credentials for requests from a web application. In the following example, requests that contain the `/mywebapp` pattern are authenticated by using Google credentials.

include::/docs/ref/feature/socialLogin/examples.adoc[tag=authfilter]
