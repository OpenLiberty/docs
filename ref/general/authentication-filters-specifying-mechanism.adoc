...
You can set authentication filters to specify the authentication mechanisms that process different types of HTTP servlet requests.
...
For example, you can set an authentication filter on social media logins to specify a particular type of social media credentials for requests from a web application. In the following example, only requests that contain the `/mywebapp` pattern are authenticated by using Google credentials.
