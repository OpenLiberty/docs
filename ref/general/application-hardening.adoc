...
This unauthorized access can lead to users having access to sensitive information about your application and other users.
...
When the `<deny-uncovered-http-methods>` element is specified, the container denies any uncovered HTTP methods that aren't listed in the security constraints for a URL pattern that best matches the request URL.
...
In the following secure example, the `<deny-uncovered-http-methods>` element is specified in `web.xml` file:
...
When the `<deny-uncovered-http-methods>` element is specified in the `web.xml` file, a message is also logged in the `messages.log` file for each URL pattern in each servlet. This message indicates the uncovered methods with a note that those methods are unprotected and not accessible, as shown in the following example:
...
This message indicates the uncovered methods with a note that those methods are unprotected and accessible, as shown in the following example:
