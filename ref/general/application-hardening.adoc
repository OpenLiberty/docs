...
You can serve servlets by class name by setting the `serveServletsByClassnameEnabled` property to `true` in the `ibm-web-ext.xmi` file of the application.
...
It's likely that you must list all the methods if you are using multiple constraints.
...
For link:/docs/ref/feature/#servlet-3.1.html[Servlet 3.1] or higher, use the `<deny-uncovered-http-methods/>` or `<http-method-omission>` elements.
...
To ensure that applications run only over secure HTTP connections, specify transport guarantee of `CONFIDENTIAL` in the `web.xml` or `ibm-web-ext.xmi` files:
