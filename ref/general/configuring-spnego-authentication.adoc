...
To enable SPNEGO, the clocks for the client, Active Directory server, and Open Liberty server must be synchronized to within five minutes of each other.
...
To configure SPNEGO, you'll complete the following items:

. Create a Kerberos service principal name and keytab file and copy the keytab file to Open Liberty.
. Create a Kerberos configuration file on Open Liberty, and link:/docs/ref/feature/#spnego-1.0.html[configure the SPENGO feature].
