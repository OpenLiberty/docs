...
Annotations that define CDI beans must be discovered. CDI annotation scanning can be enabled by using a `beans.xml` file in the META-INF folder for a `.jar` file, or in the WEB-INF folder for a `.war` file. This file can be empty, or it can contain something like the following content:
[source,xml]
----
...
----
The presence of an empty `beans.xml` file or a `beans.xml` file with `bean-discovery-mode="all"`attribute, makes all of the potential objects CDI beans. Otherwise, only objects with CDI bean-defining annotations are CDI beans.
