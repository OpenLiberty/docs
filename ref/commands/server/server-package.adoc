...
Generate a ZIP file, `myserver.zip`, that includes all files in the Open Liberty installation directory with the `myserver` server files:
...
Generate a minimal runnable JAR file of the `myserver` server named `package_myserver.jar` to the `C:\my temp` directory:
...
Package the Open Liberty runtime into a ZIP file named `myPackage.zip` and put it in `C:\temp`:
