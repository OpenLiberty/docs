...
Packages a Liberty server, its resources, and applications in a compressed file that you can store, distribute, or deploy to a different location.

// If relevant, more info.
You can select the parts of your Liberty server installation to package. You can package an application with a full or a minimal Liberty runtime environment, or without a Liberty runtime environment so that the application can be deployed on an existing Liberty installation.


== Usage examples

To generate a ZIP file, `myserver.zip`, that includes all files in the Liberty installation directory with the `myserver` server files:

----
server package myserver --include=all
----

To generate a ZIP file named `package_myserver.zip` that has only the `myserver` server files for deployment on an existing Liberty runtime installation, such as in a Docker container:
...
To package the Liberty runtime into a ZIP file, `wlp.zip`, that does not include the `usr` directory, so has no servers or applications:

----
server package --include=wlp
----

To package the Liberty runtime into a ZIP file named `myPackage.zip` and put it in `c:\temp`:
