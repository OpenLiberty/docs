...
If you've written an app that requires login, it's important to think about how to prevent fradulent access logins. Identify the IP address, and that peice of data can help track the clients where invalid logins originate. Developers are implementing HTTP Forwarded and X-Forwarded header support to establish the original IP address of a client so that your app knows what the IP address is.  The Forwarded and X-Forwarded-* header support in WebSphere Liberty is the implementation of the spec defined Forwarded header https://tools.ietf.org/html/rfc7239[RFC 7239] and the de facto standard X-Forwarded-* header.
...
== Optional Configuration of remoteIps
