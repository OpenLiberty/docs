...
If two or more basic registries are configured in your server.xml file or in a userRegistry.xml file, you can combine them into a single federated registry. However, user registries that are configured with the `quickStartSecurity` element cannot be federated with other registries. To federate two or more basic registries, you must enable both link:/docs/ref/feature/#appSecurity-3.0.html[the Application Security feature] and the Federated User Registry feature, as shown in the following example:
