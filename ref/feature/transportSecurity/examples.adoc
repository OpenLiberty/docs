...
The certified name (CN) value of the certificate `subjectDN` attribute is the host name of the machine where the server is running.
...
When the `defaultKeyStore` element exists in the configuration, Open Liberty builds a TLS configuration around it that is called `defaultSSLConfig`.
