...
Then, specify the base Distinguished Name, the host, LDAP type, port number, and realm attributes. Most LDAP servers require bind credentials to authenticate a client to the server. Bind Distinguished name and password are specified in the `ladpRegistry` element.
