...
[NOTE]
====
The `--aesConfigFile` file must contain either the `wlp.aes.encryption.key` or `wlp.password.encryption.key` variable. Only one variable can be present in a file.
====
