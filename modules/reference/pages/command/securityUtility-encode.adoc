...
Use a configuration file that contains the AES key. Specify the `--aesConfigFile` option:

[source,sh]
----
securityUtility encode --encoding=aes --aesConfigFile=aesConfig.xml MyPassword
----
**NOTE**: The configuration file must contain either the `wlp.aes.encryption.key` or the
`wlp.password.encryption.key` variable. Only one variable can be present in a file.
