...
Set the amount of time to wait for the `myserver` server to stop.

----
server stop myserver --timeout=1m
----

This sets the timeout value to 1 minute. The default timeout value is 30 seconds.
...
|--timeout
   ...
   {empty} +
   {empty} +
   The format for the duration is `XXmXXs`. Where `m` stands for minutes and `s` stands for seconds. If no units are specified, the value is assumed to be seconds.
   {empty} +
   {empty} +
   The following example shows how to set the default timeout value.
