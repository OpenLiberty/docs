...
|--include=runnable
|Use only with the `--archive="package_file_name.jar"` option. Packages an executable JAR file from which you can start an Open Liberty server by running the JAR file. See xref:runnable-jar-files.adoc[Runnable JAR files].
{empty} +
{empty} +
