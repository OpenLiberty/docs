...
* <<auditUtility auditReader, auditUtility auditReader command>>
...
The `auditUtility auditReader` command is an action of the `auditUtility` command.
You can run the `auditUtility auditReader` command with parameters to decrypt and unsign logs.
