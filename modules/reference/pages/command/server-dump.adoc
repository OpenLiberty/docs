...
serverName::
The name of the server to run the command against. If no server is specified, the `defaultServer` is used. If no existing server is named `defaultServer`, the command fails when no server name is specified.
