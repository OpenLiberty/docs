...
Starting with the 25.0.0.3 release, Liberty uses updated algorithms to sign and encrypt audit logs, regardless of whether FIPS 140-3 is enabled.
The updated link:https://openliberty.io/docs/latest/reference/command/auditUtility-auditReader.html[auditUtility] command does not read audit logs that were signed or encrypted before version 25.0.0.3. 
To read logs from these earlier versions, run the `auditUtility` command from a Liberty server version 25.0.0.2 or earlier.
