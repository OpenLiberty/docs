...
auditUtility auditReader
  ...
  --encrypted=true
  --encKeyStoreLocation=c:/wlp/usr/servers/myserver/mykeystore.jks
...
|`--signingKeyStoreLocation`=[_fully-qualified file location of keystore used to store the certificate used to unsign audit records._]|Specify the fully qualified file URL location of the keystore that stores the certificate that is used to unsign the audit records.
