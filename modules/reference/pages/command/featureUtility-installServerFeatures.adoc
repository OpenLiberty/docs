...
The `featureUtility installServerFeatures` command installs the features that are listed in the `server.xml` file of the specified server.
This command is helpful if multiple features are listed in your `server.xml` file because it prevents the need to individually list all the features that you want to install.
