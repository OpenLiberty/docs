...
serverName::
A name of the server to run the command against. If no server name is specified, the `defaultServer` server is used to checkpoint. If no `defaultServer` server exists, the command creates one and checkpoints it. 
