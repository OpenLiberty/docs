...
----
<path_to_liberty>/bin/springBootUtility thin \
    --sourceAppPath=hellospringboot.jar \
    --targetLibCachePath=<path_to_liberty>/usr/shared/resources/lib.index.cache \
    --targetThinAppPath=<path_to_liberty>/usr/servers/helloserver/dropins/spring/hellospringboot.jar
----
