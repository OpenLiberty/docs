...
|Specifies a custom AES encryption key for decrypting passwords. If not specified, featureUtility uses the Liberty default AES encryption key. featureUtility also automatically loads the encryption key from the `bootstrap.properties` or `server.env` file of the specified server when you run the `featureUtility installServerFeatures` command.
...
For security, passwords in the `featureUtility.properties` file must be encrypted. The AES encryption algorithm is recommended. Use the `securityUtility encode` command to encrypt passwords before you add them to the properties file. For more information, see xref:command/securityUtility-encode.adoc[securityUtility encode].


== Disable Maven Central fallback

By default, if custom repositories are configured but cannot be reached, the `featureUtility` commands fall back to using Maven Central. You can prevent this fallback behavior by setting the `useDefaultRepository` property to `false` in the `repositories.properties` file that's located in the `{wlp.install.dir}/etc/` directory.

When `useDefaultRepository=false` is set and a custom repository cannot be reached, the `featureUtility` command fails with an error message instead of attempting to use Maven Central.

In the following example, the `useDefaultRepository` property is set to `false` in the `{wlp.install.dir}/etc/repositories.properties` file:

----
useDefaultRepository=false
----

With this setting and a custom repository that cannot be connected in the `{wlp.install.dir}/etc/featureUtility.properties` file:

----
myRepo.url=http://alt.mycomp.com/maven2
----

The `featureUtility install` command produces output similar to the following example:
...
This behavior ensures that features are only installed from your configured repositories and prevents unintended installations from Maven Central when custom repositories are unavailable.
