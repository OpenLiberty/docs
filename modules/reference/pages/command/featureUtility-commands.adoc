...
To avoid compatibility issues, you can  for any features that are part of the Java EE, Jakarta EE, or MicroProfile platforms.
You can use the `featureUtility installFeature` and `featureUtility installServerFeatures` commands to install both versioned and xref:reference:feature/versionless-features.adoc[versionless features].
