...
The enhanced directory support introduces a check for directories that are specified in the `includes` element within the `server.xml` file. Instead of overlooking the directory, the server now reads and processes all configuration files within the directory that is specified in the `location` attribute.
...
[source,properties]
----
[AUDIT] CWWKG0028A: Processing included configuration resource: /path/to/common/a.xml
[AUDIT] CWWKG0028A: Processing included configuration resource: /path/to/common/b.xml
[AUDIT] CWWKG0028A: Processing included configuration resource: /path/to/common/c.xml
----

**Note:** Only files directly within the specified directory are processed. Files in the subdirectories are ignored to optimize startup time.
