...
The name of the file is used as the variable name, and the contents of the file are used as the variable value, as shown in the following example:
----
If you create a file in the `variables` directory named `httpPort` with a value of `9080`.
You can then use the variable `${httpPort}` in the `server.xml` file and it resolves to `9080`.
----

If a file is located in a directory within the `variables` directory, then the parent directory is added to the variable name, as shown in the following example:
----
The `variables` directory contains a directory that is named `ports`, and that directory contains a file name `httpPort` with a value of `9080`.
You can use the variable `${ports/httpPort}` in `server.xml` file and it resolves to `9080`.
----
