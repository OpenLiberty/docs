...
* The Key values must not contain any spaces.

* Empty lines are to be ignored.

* The Lines that start with the number sign # are ignored, except for the "# enable_variable_expansion" which enables variable expansion on Linux.
...
[#exp-var-windows]
==== Expansion variables
Expansion variable encapsulates the ability to access as well as manipulate the values of variables.

As of 22.0.0.1, support for variable expansion provided to the `server.env` file on Linux. Expansion variables are prefixed by a `$` and the variable name can also be enclosed in curly braces {}. For example, `$variable_name` or `${variable_name}`.

This support has always existed on Windows, though it was not documented.
