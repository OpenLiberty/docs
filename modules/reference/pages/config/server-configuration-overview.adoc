...
You can set the `location` attribute to a relative or absolute file path, a directory, or to an HTTP URL.

For more configuration files that are brought from a directory by using the `include` element, the `onConflict` attribute is applied to each file in that directory separately. For example:
...
Functionally, this is the same as including each file separately with its own onConflict attribute as shown in the following example:
...
When you are specifying a directory in the `include` location, make sure that you append a slash(`/`). For example, (location="./common/"). Omitting the slash(`/`) after the directory name (location="./common") results in an error and incorrect processing.
...
When you are including a configuration from a URI for a remote resource, it must point to a single remote resource file and cannot be a directory. For example, https://website.com/path/to/server.xml[] is supported, but including https://website.com/path/to/directory[] is not.

When the server processes configuration files within directories such as `configDropins`, it switches the file names into lowercase and sorts them alphabetically, making sure that the file processing order is consistent and predictable.
