...
Set only in the `etc/server.env` file, as other locations are relative to the `usr` directory.
...
Example for expansion variable for windows is shown in the following `server.env` file.
