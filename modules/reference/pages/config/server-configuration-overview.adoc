...
[source,xml]
----
<server description="new server" quiesceTimeout="1m30s">
    <featureManager>
...
* The `quiesceTimeout` attribute is set to `1m30s`, which gives the server up to 90 seconds to complete active work — such as finishing in-flight requests and stopping services — before shutdown proceeds. The minimum value is `30s` and the default is `30s` if the attribute is omitted.
...
You do not need to escape special characters, such as spaces.
...
The server reads this file earlier than the `server.xml` file, so it can affect the startup and behavior of the Open Liberty kernel from the start.
...
For example, this `bootstrap.include` file can contain a common set of bootstrap properties for multiple servers to use.
...
If you do, the variables that are specified on the command line override all other sources of variables and cannot be changed after the server starts:
...
For example, if you create a `${httpPort}` file in the `ports` subdirectory within the variable directory, the variable name is `${ports/httpPort}`.
...
== Configuration merging
Because the configuration can consist of multiple files, it is possible that two files provide the same configuration.
...
You can set the value of the `onConflict` attribute to `IGNORE` or `REPLACE` any conflicting configuration:
