...
In the Open Liberty server configuration files, variables are referenced by using the `${variable.name}` syntax. 
The use of variables allows you to define reusable values that can be substituted with actual values at runtime. 
`$` indicates the start of a variable reference. `{}` encloses the name of the variable. Open Liberty identifies this variable and substitutes the appropriate value.
