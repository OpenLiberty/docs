...
You can set the `location` attribute to a relative or absolute file path, a directory, or to a URI for a remote resource.
...
When the server processes configuration files within directories, it switches the file names into lowercase and sorts them alphabetically, making sure that the file processing order is consistent and predictable. This means that if two files are specified in the same directory that have the same name except for case variations (such as `extraConfig.xml` and `ExtraConfig.xml`), the ordering behavior is indeterminate.
