...
The directory support introduces a check for directories that are specified in the `includes` element within the `server.xml` file. Instead of overlooking the directory, the server now reads and processes all configuration files within the directory that is specified in the `location` attribute.
