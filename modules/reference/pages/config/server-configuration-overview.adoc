...
If this environment variable is not specified, the default location for the variables directory is `${server.config.dir}/variables`.
...
If the name of the file ends in `.properties`, each property in the file is used to create a variable. For example, if the `variables` directory contains a file that is named `ports.properties` with the contents:
----
. httpPort=9080
. httpsPort=9443
----
The variables `${httpPort}` and `${httpsPort}` can be used in the `server.xml` file and the values resolves to `9080` and `9443`.
