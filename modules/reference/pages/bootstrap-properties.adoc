...
You can use the `bootstrap.properties` file to create custom default HTTP and HTTPS properties.
...
You can use the `default.http.port` and `default.https.port` properties specified in the `bootstrap.properties` file as custom properties from the `server.xml` configuration file.
...
For more information, see xref:server-configuration-overview.adoc#variable-substitution[Variable substitution precedence].
