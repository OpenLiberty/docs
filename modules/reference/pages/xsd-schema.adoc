...
Download the Open Liberty binary files and extract the archive, which can be found in the newly created `wlp` directory.
...
The command saves the generated XSD schema to `/wlp/usr/defaultServer/server.xsd` file.
Create the `defaultServer` directory by starting the Open Liberty instance, if the directory does not exist.
...
The default location for the `server.xml` file for embedded Open Liberty is `src/main/resources/liberty/config`.
