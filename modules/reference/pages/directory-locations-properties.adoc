...
|{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp} `+- health/`
|
|When file-based health checks are enabled for the feature:mpHealth-4.0[display=mpHealth-4.0] feature, the `health/` directory is created to contain the `started`, `live`, and `ready` health check files.
{empty} +
{empty} +
This directory and its contents are deleted when the server shuts down or when the `mpHealth` feature is unloaded.
For more information about file-based health checks, see xref:ROOT:health-check-microservices.adoc#_microprofile_health_v4_0[file-based health check overview].
