...
The feature:concurrent-3.1[display=Jakarta Concurrency 3.1] (`concurrent-3.1`) adds support for Java 21 virtual threads, scheduled asynchronous methods, and integration with CDI and Java Flow / Reactive Streams.
...
* **Reactive Streams support**: `ContextService` adds support for Java Flow types so that Jakarta EE contexts propagate when working with Reactive Streams.
...
* References to **Concurrency Utilities for Java EE** are updated to **Jakarta Concurrency** throughout the feature.

* For Jakarta EE 11 applications, `concurrent-3.1` is the recommended feature version in `server.xml`.

=== Restrictions

* The existing restriction where certain security-related information is not propagated into the submitter’s subject remains in effect for all Jakarta Concurrency feature versions.

For more information, see the https://jakarta.ee/specifications/concurrency/3.1/jakarta-concurrency-spec-3.1[Jakarta Concurrency 3.1 Javadoc].
