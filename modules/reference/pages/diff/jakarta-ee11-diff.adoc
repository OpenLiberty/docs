...
=== Java Security deprecation in Liberty

Liberty no longer supports the use of a `SecurityManager` to enable Java Security when you use Jakarta EE 11 features, which aligns with the removal of Java Security support in the Jakarta EE 11 platform. The Liberty runtime automatically disables the `SecurityManager` on Java SE 17 if Java Security is configured in the server. Liberty already disables the `SecurityManager` by default for Java SE 18 and subsequent releases.
...
These classes replace the dependency on the `java.security.Policy` class, which is deprecated in newer versions of Java.

* **Per-context policies:** With the new `PolicyFactory` API, you can now have a `Policy` per policy context instead of having a global policy. 
...
This design allows an application to have a different configuration than the server-scoped one, but still allows it to delegate to a server-scoped factory for any other applications. 
...
* **Action Required:** If a Liberty user feature configures authorization modules, the OSGi service that provided a `ProviderService` implementation must be updated to use the `PolicyConfigurationFactory` and `PolicyFactory` set methods in the OSGi component. 
...
* **Error Message:** When using an existing user feature that uses the `ProviderService` API, your user feature will fail to start with an error message that states `Unresolved requirement: Import-Package: com.ibm.wsspi.security.authorization.jacc; version="[10.0.0,20.0.0)"`.

For more information, see the https://jakarta.ee/specifications/authorization/3.0/apidocs/jakarta.authorization/module-summary.html[Jakarta Authorization 3.0 Javadoc].
