...
MicroProfile OpenAPI 4.1 adds the `getJsonSchemaDialect()`method and the `setJsonSchemaDialect()` method to set the default `jsonSchemaDialect` field by using the model API. 
...
MicroProfile Telemetry 2.1 equips you with the most recent Open Telemetry technology. This feature now uses OpenTelemetry-1.48.0, upgraded from 1.39.0. 
