...
- <<#uriinfo-path,UriInfo.getPath() leading slash>>
- <<#getrequestheader-absent,HttpHeaders.getRequestHeader() return value for an absent header>>
...
[#getrequestheader-absent]
=== HttpHeaders.getRequestHeader() return value for an absent header
