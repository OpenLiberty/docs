...
For more information, see the link:https://jakarta.ee/specifications/security/3.0/apidocs/jakarta.security/module-summary.html[Jakarta Security 3.0 Javadoc] and the xref:ROOT:enable-openid-connect-client.adoc[Enable an OpenID Connect client for an application] topic.
...
The feature:xmlWS-4.0[display=Jakarta XML Web Services 4.0] feature includes changes for Global Handlers and the Jakarta Web Services Metadata specification. It also removes a previously deprecated class for the SOAP with Attachments 3.0 API, which is supported in Open Liberty by the Jakarta XML Web Services feature.

* With the 4.0 release, the Jakarta Web Services Metadata specification is merged into the Jakarta Web Services specification. All the Jakarta Web Services APIs for Open Liberty are now included in the `dev\api\spec\io.openliberty.jakarta.xmlWS.4.0...jar` file.
