...
If a service in your microservice application is not ready to migrate the trace system from MicroProfile OpenTracing to MicroProfile Telemetry, you can still us MicroProfile Telemetry with your other services. To use both MicroProfile OpenTracing with MicroProfile Telemetry, just set a compatible propagator. Propagators are mechanisms that enable the movement of telemetry data among different services by using HTTP headers.
...
By default, MicroProfile OpenTelemetry uses Trace-Context headers. To export distributed traces successfully to the Zipkin backend exporter, service B must include the B3 propagation header for Zipkin by specifying either the `OTEL_PROPAGATORS=b3` environment variable or the `otel.propagators=b3` configuration property.
