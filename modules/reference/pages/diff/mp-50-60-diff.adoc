...
If you are not ready to migrate your trace system from OpenTracing to OpenTelemetry in a service, you can combine OpenTracing with  OpenTelemetry so your other services are not restricted. To combine OpenTracing with OpenTelemetry, you need to add configuration to set a compatible propagator.

Propagators are mechanisms that make moving telemetry data between different services possible.

Several different standards are available to propagate span information between microservices by using HTTP headers. By default, OpenTelemetry uses Trace-Context headers. The `otel.propagators` configuration property configures which propagators are used.

For a list of the propagation protocols that OpenTelemetry supports, see the <<#select,Selecting a compatible propagator>> section.

.Microservice tracing example
image::mptel.png[diagram that shows the flow of traces to Zipkin from two microservices]


The preceding diagram shows two microservices that enable tracing, with traces that are collected and exported to Zipkin. The services can collect traces with either MicroProfile OpenTracing or MicroProfile Telemetry without losing the parent-child relationship in the spans.

In this example, service A is traced with MicroProfile OpenTracing, while service B is traced with MicroProfile Telemetry. To export distributed traces successfully to the Zipkin backend exporter, service B must specify either the `OTEL_PROPAGATORS=b3` environment variable or the `otel.propagators=b3` configuration property and include the B3 propagation header for Zipkin.

If more than one propagator is enabled, then headers are added to outgoing requests and any header is accepted for incoming requests. For example, if you are migrating from using a Jaeger client, you might set the `otel.propagators=jaeger` configuration property to use the Jaeger propagation protocol. Alternatively, you might set the `otel.propagators=jaeger,tracecontext,baggage` configuration property to allow use of both the Jaeger and W3C protocols.
