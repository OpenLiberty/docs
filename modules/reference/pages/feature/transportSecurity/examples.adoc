...
|Password
|The default password is the password that is specified in the keystore element. If no password is provided in the keystore element, the keystore password in the `server.env` file is used.
...
You can run the xref:reference:command/securityUtility-createSSLCertificate.adoc[securityUtility createSSLCertificate] command to create and customize the default certificate and keystore.
