...
At startup, Open Liberty can read certificates from a file or environment variable so that they can be used to establish trust. Set the `cert_(name of existing truststore)`environment variable to the value of the certificate or the path to the file that contains one or more certificates. The environmental variable must reference a certificate in the base 64-bit format or the path to a file that contains a base 64-bit encode or DER encoded certificate.
