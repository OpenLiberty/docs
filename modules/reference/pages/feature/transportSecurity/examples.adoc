...
=== Configure multiple SSL protocols

SSL protocols can be configured by using the `sslProtocol` attribute. Multiple SSL protocols can also be configured on the `sslProtocol` attribute if the underlying JDK support exists. Use a comma-separated list from the TLSv1, TLSv1.1, TLSv1.2, and TLSv1.3 protocols values.
