...
[#configure-specific]
=== Configure specific SSL protocols

Configure SSL protocols on the `sslProtocol` attribute if the underlying JDK support exists for the protocols. Create a comma-separated list from the `TLSv1`, `TLSv1.1`, `TLSv1.2`, and `TLSv1.3` protocol values. This list of protocols overrides the TLS protocols that are enabled in the default TLS configuration.
