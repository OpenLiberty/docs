...
1. A `library` element that specifies tyhe location of the JDBC driver file or files
2. One or more `dataSource` elements that reference the `library` element
...
A Servlet or EJB can then get it injected for use with code similar to the following exaple:
...
To connect with to remote databases, you typically must provide credentials.
