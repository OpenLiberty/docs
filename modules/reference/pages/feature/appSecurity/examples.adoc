...
[Note]
====
If a `keyPassword` is not configured in the `ltpa` element in the `server.xml` file, and if the `ltpa_keys_password` and `keystore_password` environment variables are not set in the `server.env` file, such as when the `--no-password` option is specified with the xref:reference:command/server-create.adoc[server create] command, then the LTPA service will fail.
====
