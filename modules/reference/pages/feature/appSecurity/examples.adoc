...
When both the Application Security and feature:servlet[Jakarta Servlet] features are enabled, you can configure an Open Liberty server to track logged0-out LTPA tokens. The Application Security feature version 3.0 and later enables the Jakarta Servlet feature by default.
...
However, you can configure the server to track logged-out LTPA tokens so that if they are presented again on the same server, the user must authenticate again.
