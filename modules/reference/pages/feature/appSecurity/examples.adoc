...
You can configure the authentication to use a distributed cache, which has benefits for performance and fail-over recovery when a group of servers share the same users and groups. For more information, see xref:ROOT/distributed-authentication-cache.adoc[Configure a distributed authentication cache].
