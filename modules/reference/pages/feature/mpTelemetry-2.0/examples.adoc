...
[source,xml]
----
<mpTelemetry source="message, trace, ffdc, accesslog"/>
----
...
To enable OpenTelemtry to collect access log events, configure access logging in the server configuration and the `accessLog` source in the source list, as shown in the following example:
...
The Open Liberty feature:audit[display=Audit] feature captures security-related events from the runtime environment and emits human-readable audit records.
To collect and send audit logs to your configured OpenTelemetry exporter, add either the `audit-1.0` or `audit-2.0` feature and specify the `audit` log source, as shown in the following example:
...
You can configure different audit events that are captured and routed to OpenTelemetry by specifying the relevant audit events and outcomes in the config:auditFileHandler[] element. For more information, see link:/docs/latest/reference/feature/audit-2.0.html#_configure_the_audit_events_to_log[Configure the audit events to log] in the Audit feature documentation.
...
In this example, the versionless `mpHealth` and `mpMetrics` features are automatically resolved to compatible versions by the `microProfile-5.0` platform. The versioned `mpTelemetry-2.0` feature gets the benefit of Java SE 8 support that the platform provides.
