...
As a best practice, you can specify the `federatedRepository` element to federate multiple LDAP registries. Specifying the `federatedRepository` element is not mandatory, as they are federated automatically.
...
After the mapping is configured, when you use the user registry attribute for any operation, the value will be equivalent to the value of the LDAP attribute that is mapped.
...
The following example shows both the primary LDAP server that is specified in the `ldapRegistry` element. Two sets of LDAP `failoverServers` elements are specified.
These `failoverServers` elements have multiple `server` elements that are defined within them. These `server` elements act as the backup servers in case the primary LDAP servers go offline.
