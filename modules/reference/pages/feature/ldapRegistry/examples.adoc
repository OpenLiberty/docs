...
The following example shows a basic user registry with an administrator role mapping that is federated with an LDAP user registry, with the configuration specified in the `federatedRepository` element:
