...
You can specify a global Kerberos configuration in your `server.xml` file that can provide values to any Open Liberty features that use Kerberos credentials. This global configuration is optional. For more information, see xref:ROOT:kerberos-authentication.adoc[Kerberos authentication for Open Liberty].
