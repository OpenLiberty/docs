...
The `krb5TicketCache` attribute is optional and specifies the location of a ticket or credential cache (`ccache`) file. When this attribute is specified and the principal is authenticated, the Kerberos service automatically attempts to renew the Kerberos ticket before it expires.
