...
If the `krb5TicketCache` attribute is not specified, Open Liberty resolves credential values from the globally configured the `keytab` file that is configured in the global `kerberos` element. If no `keytab` file is configured, Open Liberty resolves credential values from the default Kerberos credential cache location for the Java SDK or the operating system. If both the `krb5TicketCache` attribute and the `keytab` attribute from the global `kerberos` element are configured, Open Liberty searches for credentials first in the `ccache` file and then in the `keytab` file.
