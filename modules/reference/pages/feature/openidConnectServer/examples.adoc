...
The example uses the xref:feature/appSecurity[Application Security] feature to manage a single user that is defined in the server configuration.
...
The example configuration uses a local store to store the client data and token status.
...
=== Managing OAuth clients with an authenticated database in production
