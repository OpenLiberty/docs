...
To enable to the following configuraition, in addition to the Cloudant Integration feature, you must also have the feature:restConnector[display=Admin REST Connector feature] and the feature:appSecurity[display=Application Security feature] enabled in your `server.xml` file:
