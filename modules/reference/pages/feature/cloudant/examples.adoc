...
To enable the following configuration, in addition to enabling the Cloudant Integration feature, you must also enable the xref:feature/restConnector.adoc[Admin REST Connector feature] and the xref:feature/appSecurity.adoc[Application Security feature] in your `server.xml` file:
