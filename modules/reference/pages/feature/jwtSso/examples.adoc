...
If the request meets the criteria configured in the JWT SSO authentication filter, then you can authenticate with the JWT to access the protected resource.
Conversely,  if the request does not meet the criteria configured in the JWT SSO authentication filter, then you are prompted to provide login credentials.
