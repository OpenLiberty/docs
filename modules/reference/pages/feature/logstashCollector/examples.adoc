...
In the `server.xml` file, you can also limit the maximum number of events that are sent for each type of event per second. The default maximum number of events is 0, which means that no throttling is applied. If the `maxEvents` parameter is set to greater than 0, the number of events that are sent per second is limited. The `maxEvents` parameter value must be an integer through 0 and 2147483647.
