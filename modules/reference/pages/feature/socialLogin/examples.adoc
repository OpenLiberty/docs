...
The following example shows how to configure your application to request that the user logs in with their Google ID.
  In this example, `your_app_id` and `your_app_secret` are the Google client ID and client secret credentials for the application that are defined in the `application` element.
