...
=== Request log in with a social media ID for a subset of applications, URLs, browsers, or IP addresses
...
You can configure the `socialLogin-1.0` feature to use OpenShift service accounts for authentication and authorization of protected resource requests.
...
The `okdServiceLogin` element authenticates all protected resource requests received by the Open Liberty server, by default.
