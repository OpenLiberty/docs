...
In the case of users logging in with their Google ID, the Google client ID is "your_app_id" and the Client Secret is "your_app_secret".
...
[source,xml]
----
<!-- user will be presented a choice menu of either Google or ldap -->
...
For more information, sesee the config:authFilter[display=authFilter element].
...
// LC: Leaving the following links in the source, for now, to show where this topic should link to when the relevant equivalent topics are published in the Open Liberty docs (do not link to the KC from Open Liberty docs). Remove this commented section when the relevant links are added in the future.
...
The socialLogin feature can be configured to use OpenShift service accounts to authenticate and authorize protected resource requests.
