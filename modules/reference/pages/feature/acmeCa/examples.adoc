...
You can make HTTPS calls to the REST API to renew and revoke certificates and get details about the active CA account. To use the REST APIs to view, renew or revoke the certification, you must be in the administrator or reader role. For more information, see xref:root:acme-cert-management.adoc_acme_ca_account_management[ACME account management]. The following configuration includes a basic registry with an administrator role:
