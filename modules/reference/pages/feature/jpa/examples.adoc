...
You can specify Eclipse-specific log categories by using the following syntax for the `traceSpecification` attribute: `eclipselink.{log-category}=all`. For example, to enable EclipseLink SQL trace, you can specify configuration similar to the following example:
