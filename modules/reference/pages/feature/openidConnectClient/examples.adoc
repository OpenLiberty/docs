...
When using asymmetric algorithms and a trust store configuration, the client uses the following strategies to locate the appropriate public key for signature verification:

- With `FROM_HEADER`: The public keys must be prefixed with their corresponding algorithm for automatic selection. The client identifies the `alg` claim from the token header and searches the trust store for a key alias beginning with that value. For example, the alias `rs256_key` will be used for tokens signed with `RS256`. If no algorithm-prefixed key is found, the client falls back to using the key specified by the `trustAliasName` attribute.

- Without `FROM_HEADER` (Single Algorithm): The client only uses the key specified by the `trustAliasName` attribute for signature verification.

Alternatively, for either of the above configurations, you can configure JWK-based verification as described in xref:ROOT:json-web-token.adoc[Sign and verify JSON Web Tokens with JSON Web Keys].
...
Use of ECDH algorithms requires the use of EC keys. See the section titled <<#gen-ec-keys,Generate EC keys for ECDH>>.
...
Note that this is just an example, and you might need different constraints based on your application.

This will add the constraints to the already registered Sun and SunJCE providers to allow for RSA-OAEP key agreement algorithm. The constraints to be added will be discovered through the troubleshooting process as described in link:https://www.ibm.com/docs/SSAW57_liberty/com.ibm.websphere.wlp.nd.multiplatform.doc/ae/twlp_fips_troubleshoot.html[Troubleshooting FIPS 140-3 on Liberty]. For more information about the custom profile file, see link:https://www.ibm.com/support/pages/node/7113528#creatingextensionsofprofiles[Creating extensions of profiles].

[#gen-ec-keys]
===== Generate EC keys for ECDH

To use ECDH, you need to configure the EC keys in the keystores.
