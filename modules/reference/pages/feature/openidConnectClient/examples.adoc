...
If a user's group information is not found in the ID token, you can configure Open Liberty to also check for this information in the Access token and `userinfo`. Set the `tokensOrderToFetchCallerClaims` attribute to `AccessToken IDToken Userinfo`, as shown in the following example.
...
In this example, Open Liberty checks the Access Token first, then the ID token, and lastly, `userinfo`. If no group information is found in these locations, an unauthorized response code is returned.
