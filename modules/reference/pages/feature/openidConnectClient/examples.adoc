...
Open Liberty allows by using the issuer claim from a JWT or JWS access token to select the `openidConnectClient` configuration with a matching `issuer` attribute.

An issuer claim value of `"https://hostname/op1"` and a `server.xml` file that is configured with the `RP1` configuration is selected if the `"rp2filter"` does not apply to the request.
...
To find which `openidConnectClient` applies to the request is done by using the following algorithm.
+
- If the auth filter for a `openidConnectClient` matches the request, choose that configuration.
- Check the issuer from the JWT against the issuers of all configuration elements. If the issuer from the JWT matches a single `openidConnectClient` configuration element, choose that configuration.
- If the issuer from the JWT matches the issuer of multiple `openidConnectClient` configuration elements, choose the first configuration matched.
- If the issuer does not match the issuer of any configuration elements, choose between all of the configuration elements in a nondeterministic manner.
+
