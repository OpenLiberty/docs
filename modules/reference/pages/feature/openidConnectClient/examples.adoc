...
The client identifies the `alg` claim from the token header and searches the trust store for a key alias beginning with that value. For example, `rs256_keyalias` for `RS256` tokens or `es384_keyalias` for `ES384` tokens.
If no algorithm-prefixed key is found, the client falls back to using the key specified by the `trustAliasName` attribute.

When not using `FROM_HEADER` (using a single algorithm), the client only uses the key specified by the `trustAliasName` attribute for signature verification. Alternatively, you can configure JWK-based verification as described in xref:ROOT:json-web-token.adoc[Sign and verify JSON Web Tokens with JSON Web Keys].
