...
- If the `audiences` attribute is not configured, but the JWT contains an `aud` claim that is a valid URL, the resources service URL must include the `aud` value as a prefix.
+
...
The Open Liberty OIDC client automatically creates an SSO token after the ID Token is processed. You can configure the server to not create an SSO by specifying the `disableLtpaCookie` attribute for the `openidConnectClient` element.
