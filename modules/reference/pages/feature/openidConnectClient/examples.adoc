...
You can use the `OpenIdAuthenticationMechanismDefinition` annotation in a web or RESTful Web Services (JAX-RS) application to enable an OpenID Connect client for that application. For more information, see xref:enable-OpenID-Connect-client.adoc[Enable an OpenID Connect client for an application].
