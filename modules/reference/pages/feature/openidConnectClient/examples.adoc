...
In this example, Open Liberty checks for `userIdentifier` and `groupIdentifier` values first in the access token, then the ID token, and finally calls the `UserInfo` endpoint. If the values are not found in any of those locations, an unauthorized response code is returned.
...
* https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization[MCP Authorization specification]
