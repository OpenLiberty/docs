...
Converting an injected value to one of these types of properties might throw the exception `SRCFG02006: The property yourProperty cannot be converted to class java.util.concurrent.atomic .AtomicInteger/AtomicLong`. If you need to convert a String to `AtomicInteger` or `AtomicLong`, you can convert to `Integer` or `Long`. Then, you can create an `AtomicIneger` or `AtomicLong` String.
