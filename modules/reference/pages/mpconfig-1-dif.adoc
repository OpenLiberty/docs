...
Any MicroProfile Config functionality that Open Liberty implemented beyond the scope of the specification is lost from MicroProfile Config 1.x to 2.0.
...
** In MicroProfile Config 1.x to 2.0, Converters are provided for `AtomicInteger` and `AtomicLong` properties.

* In MicroProfile Config 2.0, caching is discontinued.
  Values that are looked up from MicroProfile Config are no longer cached.

** In MicroProfile Config 1.x to 2.0, requests made for the same configuration key return the same object up to 5-seconds duration, without calling the configuration sources.


* When a converter for a specific type is not available, converters of its subtypes cannot be used.

** Converters for subtypes are available for MicroProfile Config 1x to 2.0.
For example, in MicroProfile Config-1.x to 2.0, a Converter that is registered for the type `Dog` extends a type `Animal`.
