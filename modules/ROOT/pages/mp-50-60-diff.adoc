...
The MicroProfile OpenAPI 3.1 release introduces the `mp.openapi.scan.beanvalidation` MicroProfile Config property, which can be set in any of the xref:external-configuration.adoc#default[configuration sources that are available to MicroProfile Config]. This property enables or disables the scanning and processing of Jakarta Bean Validation annotations. The default value is `true` but you can disable the scans by setting it to `false`. For more information, see xref:microprofile-config-properties.adoc#openapi[MicroProfile Config properties: MicroProfile OpenAPI].
