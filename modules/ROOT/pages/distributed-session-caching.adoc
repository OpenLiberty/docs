...
If the `dbThing` object is not found in session, it is obtained from the database and cached for future use.
...
However, you don’t need to specify a Hazelcast configuration file if the default Hazelcast configuration works for you.
 You can optionally create a Hazelcast configuration file, as shown in the following example, and xref:reference:feature/sessionCache-1.0.adoc[add a reference to the Hazelcast configuration file] in your `server.xml` file:
