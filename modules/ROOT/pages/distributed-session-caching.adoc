...
If your application already caches HTTP session data, you don't need to make any code changes in the application to use the JCache Session Persistence feature.
...
The following example Hazelcast configuration file customizes how Hazelcast defines the cluster:
...
When you use distributed session caching, all objects stored as HTTP session attributes must implement `java.io.Serializable`, including all objects in the class hierarchy of those attributes.
Liberty can only persist serializable attributes to the distributed cache.
Liberty stores non-serializable attributes only in the local in-memory cache.
These attributes are lost when the server stops or fails.

Liberty logs the following warning when a non-serializable attribute is detected:

----
BackedSession putValueGuts Attribute with name <attributeName> is not Serializable. This attribute is not persisted on failover.
----

Verify that the class for the named attribute and all classes it references implement `java.io.Serializable`.
