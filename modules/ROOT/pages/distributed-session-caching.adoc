...
xref:reference:sessionCache.adoc#_minimum_configuration_to_persist_session_data_with_hazelcast_as_the_jcache_provider[Configure] the JCache Session Persistence feature and reference the library that cotains the JCache implementation.
...
You can optionally create a Hazelcast configuration file, as shown in the following example, and xref:reference:sessionCache.adoc#_minimum_configuration_to_persist_session_data_with_hazelcast_as_the_jcache_provider[add a reference to the Hazelcast configuration file] in your `server.xml` file:
