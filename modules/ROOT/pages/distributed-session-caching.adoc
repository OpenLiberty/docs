...
Each server has a local session cache stored in memory on the server.
By using an underlying JCache provider like Hazelcast those sessions are persisted.
So a new server starting up in a cloud environment would have access to sessions created by other servers.
