...
To expose a method to AI agents, annotate the method with {javadoc-base}/org/mcpjava/server/tools/Tool.html[@Tool] inside a CDI-managed bean.
...
NOTE: The class must be annotated with a CDI scope such as `@ApplicationScoped` or `@RequestScoped` for tools to be discovered. Liberty does not discover tools in unannotated classes.
...
When you return a POJO from a tool method, Liberty serialises it to JSON by default. To control exactly how a type is encoded, implement {javadoc-base}/org/mcpjava/server/ContentEncoder.html[ContentEncoder<T>] as a CDI bean. Implement `getType()` to specify which type the encoder handles. Implement `encode()` to produce a `ContentBlock`:
