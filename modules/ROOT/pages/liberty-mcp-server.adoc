...
This guide walks through the most common tasks, from registering your first tool to advanced topics such as async execution, structured content, and endpoint configuration.
...
Some features — such as `@Schema`, `DefaultValueConverter`, `ToolManager`, and `ToolResponseEncoder` — are provided by the `io.openliberty.mcp` jar that ships with Liberty. To make these available on the build path, locate the `io.openliberty.mcp_*.jar` in `<wlp>/dev/api/ibm/`, note the version suffix (for example, `1.0.106`), and add a `system`-scoped dependency:
...
==== Other Exceptions
Any exception type that is *not* listed in `@WrapBusinessError` (and is not a `ToolCallException`) is treated as an unexpected server error — the runtime logs the full stack trace and returns a generic error response to the AI.
...
Long-running tools should accept a {javadoc-base}/org/mcpjava/server/Cancellation.html[`Cancellation`] parameter. The runtime injects it automatically — it is never supplied by the AI. Call `cancellation.check().isRequested()` periodically and throw `Cancellation.OperationCancelledException` to stop execution cleanly:
