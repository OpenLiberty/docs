...
If you intend to use Micrometer v1.13.x or higher and require the Prometheus registry library, two library options are available:

* `micrometer-registry-prometheus`
* `micrometer-registry-prometheus-simpleclient`

`micrometer-registry-prometheus` 1.13.x and later introduces breaking changes. Providing this library with its dependencies is not supported, and the MicroProfile Metrics runtime will not be available.

When using Micrometer v1.13.x or later, use the `micrometer-registry-prometheus-simpleclient` library and its dependencies instead.
When using Micrometer v1.12.x or earlier, the `micrometer-registry-prometheus` library can be used directly.
====
