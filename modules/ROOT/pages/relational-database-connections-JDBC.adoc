...
For example, the following example shows `currentLockTimeout` property on the Db2 JDBC driver's data source classes:
...
You can configure this setting with the following `server.xml` file configuration:
