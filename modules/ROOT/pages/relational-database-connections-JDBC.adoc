...
The following example shows the basic configuration to specify JDBC vendor properties in the link:docs/latest/reference/config/dataSource.html#dataSource/properties[properties] subelement:
...
For an example of using a vendor-specific properties subelement, the following code shows the `currentLockTimeout` property on the IBM Db2 JDBC driver data source classes:
