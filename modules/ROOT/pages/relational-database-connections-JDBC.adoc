...
For example, the following code shows the `currentLockTimeout` property on the IBM Db2 JDBC driver's data source classes:
...
After you configure a data source, you can quickly test the configuration to see whether your Open Liberty server can access your database. For more information, see xref:testing-database-connections.adoc[Validating server connections].
...
If you use the JDBC feature version xref:reference:feature/jdbc-4.3.adoc[4.3] or higher, or you are referencing <<#default,the default data source>>, Open Liberty looks for the data source type in the following order:

. `javax.sql.XADataSource`
. `javax.sql.ConnectionPoolDataSource`
. `javax.sql.DataSource`

If you use the JDBC feature version xref:reference:feature/jdbc-4.2.adoc[4.2], xref:reference:feature/jdbc-4.1.adoc[4.1], or xref:reference:feature/jdbc-4.0.adoc[4.0] and you are not referencing the default data source, Open Liberty looks for the data source type in the following order:
