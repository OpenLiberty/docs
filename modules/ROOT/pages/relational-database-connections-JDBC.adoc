...
In this example, the `host1` and `port1` values are for the first Oracle RAC node, and `host2` and `port2` values are for the second Oracle RAC node. If you are not using Oracle services, then the value for `SERVICE_NAME` is your database name. If you are using Oracle services, then the value for `SERVICE_NAME` is the name of the services. You are not required to specify the Oracle login credentials as Oracle properties, other methods of database authentication also work. In version 19.0.0.4 and later, some Oracle RAC functions require the use of Oracle UCP.
