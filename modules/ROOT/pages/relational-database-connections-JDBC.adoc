...
If you use Maven or Gradle to build your application, you configure a dependency to copy the JDBC driver files to the `${server.config.dir}/jdbc` directory when your application is built. The driver files must be copied after the server is created but before the application is deployed so that the database instance is available to the  application at run time.
...
If you use Gradle to build your application, you can add a https://docs.gradle.org/current/dsl/org.gradle.api.tasks.Copy.html[Copy task] to your `build.gradle` file. Then, add a dependency for that task to the `deploy` task in the `build.gradle` file so that the `copy` task runs after the server is created but before the application is deployed to the server:
