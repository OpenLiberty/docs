...
If you use Maven to build your application, you can download and deploy the JDBC driver by adding code that is similar to the following example to your `pom.xml` file. In this example, the `com.ibm.db2` dependency is copied to the `jdbc` directory in the `${server.config.dir}` directory at build time to deploy the IBM Db2 JDBC driver:
