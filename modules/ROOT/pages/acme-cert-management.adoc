...
The following sections describe common problems you might encounter with ACME CA accounts and connections. For configuration examples, see the acmeCA client feature (link pending).
...
If the server cannot fetch a certificate, a keystore is still created. Due to a fixed Java bug, an empty keystore file can cause an exception. Examples of this error include the following messages:
----
...
To resolve this error, remove the empty keystore. Update to a Java level that includes the fix to allow empty keystores. For more information, see Java bug IJ19292.
