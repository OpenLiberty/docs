...
Set the `JAVA_HOME` environment variable in the `server.env` file to ensure that Open Liberty can find a JVM when the service starts. For more information about setting environment variables, see the xref:reference:config/serverConfiguration.adoc[Server configuration overview]. Run these commands from `path_to_liberty/bin` by using the following syntax, where `serverName` is the Windows service display name and the name of the server to run the command against:
