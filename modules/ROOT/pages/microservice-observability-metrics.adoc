...
Throughout the many releases of MicroProfile Metrics different metric types were introduced, removed, or modified.
...
== Tuning Histograms and Timers

Starting in MicroProfile Metrics 5.0, you will be able to adjust the the percentile precision of the `Timer` and `Histogram` metrics using the MicroProfile Config properties `mp.metrics.smallrye.histogram.precision` and `mp.metrics.smallrye.timer.precision` respectively. The property accepts a value from 1 to 5 and is defaulted to 3 if no value is specified. A greater value will result in more exact percentile calculations, but at a greater memory cost.
