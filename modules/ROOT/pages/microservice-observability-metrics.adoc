...
You can narrow down the scope of the metrics by using the "scope" query parameter with the `/metrics` endpoint to specify the metric registry scope. For example, to view the metric data for the `base` metric registry, use the `/metrics?scope=base` endpoint. This pattern is used to access all runtime provided scopes (i.e. `base`,`application` and `vendor`) and any <<customscope,  user-defined metric registry scopes>>. +
...
**MicroProfile Metrics runtimes that are version 4.0 and earlier** can also export metric data in JSON format through the `/metrics` endpoints. JSON formatted metric data can be obtained by confiuring the `Accept` header of your request to the `application/json` value.
...
Starting in MicroProfile Metrics 5.0, in addition to the default `application` metric registry scope, you are now able to define your own metric registry scope to register metrics to. This can be achieved by specifying the `scope` parameter for metric annotations or by injecting a `MetricRegistry` with the `RegistryScope` annotation and specifying the `scope` parameter. Not defining a value for the `scope` parameter in the metric annotation or `RegistryScope` annotation will default to the `application` scope.

Example using the `@Counted` metric annotation to define a user-defined scope
----
...
Example injecting a `MetricRegistry` for instrumenting metrics with the Java API.
----
@Inject
...
The following list lists metric types available in the **latest** MicroProfile Metrics feature.
...
The following list lists metric types available in previous MicroProfile Metrics releases. Metric types that are found both lists indicates that its behavior and/or capability has changed.
...
Note: When viewing the Prometheus formatted metric data for a histogram, the mean value is not included.
...
[#meter]
=== Meter [MicroProfile Metrics 1.0-4.0]
...
[#simpletimer]
=== Simple timer [MicroProfile Metrics 3.0-4.0]
...
[#timerOld]
=== Timer [MicroProfile Metrics 1.0-4.0]
...
[#histogramOld]
=== Histogram [MicroProfile Metrics 1.0-4.0]
A histogram is a metric that calculates the distribution of a value. It provides the following information:
...
Metrics instrumented with MicroProfile Metrics can be instrumented with key-value tags. The key is the tag name and the value can be referred to as the tag value. This allows for the instrumentation of multiple metrics, of the same type, using the same metric name as long as the set of tags have matching tag names, but with different values. If the metric types used or if the set of tag names do not match, then an `IllegalArgumentException` will be thrown. This is useful for logically grouping metrics which track similar statistics by using the same name and are then identified by their tags.

For example, using counters, we can create multiple counters named "methodCounter" which returns the count of the method identified by the tag.
