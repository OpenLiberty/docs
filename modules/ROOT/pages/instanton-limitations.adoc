...
This `Servlet` example uses the `loadOnStartup = 1` attribute, which, when you are using the `afterAppStart` option, initializes the servlet before the checkpoint. The runtime detects this conflict and logs the following message:
