...
- <<#mp-config-no-default, Accessing MicroProfile Configuration with no default value at Checkpoint>>
...
== Accessing MicroProfile Configuration with no default value at Checkpoint
An application injected with a configuration property that has no default value set in any configuration source during checkpoint could cause some errors. This section provides the different types of error that are encountered and the remedy to fix those errors.
