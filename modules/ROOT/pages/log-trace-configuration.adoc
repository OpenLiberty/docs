...
* <<#json,JSON logging>>
  ...
  ** <<#http,Custom HTTP access log JSON fields>>
...
You can add custom fields to your JSON-formatted message and trace output to gather information about a particular issue or incident. For example, if you want to check the requests made from a specific user, you can add a custom field to filter application logs by that user’s ID. Furthermore, it might be useful to have another field for the session ID so that you can analyze and filter application logs for a specific session.
...
The newly specified field is added to log and trace entries that are created on the same thread. In this example, a custom field that is called `userName` is added for the `bob` user ID.
...
|When logs are in JSON format, you can use the `jsonAccessLogFields` attribute to replace the default HTTP access log JSON fields with fields that are specified by the `logFormat` attribute of the `accesLogging` element.
