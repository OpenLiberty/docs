...
The TBASIC logging format provides a consistent log format across your Open Liberty console, message, and trace files.
A BASIC logging format exists for `traceFormat`.
However, the output differs between `traceFormat` and that of the `consoleFormat` and `messageFormat`.
The TBASIC format adds consistency to `consoleFormat`, `messageFormat`, and `traceFormat` to match the BASIC option for `traceFormat`.
...
You can specify the TBASIC log format in your `bootstrap.properties` file, as shown in the following example:
...
|This setting specifies the required format for the console. Valid values are `dev`, `simple`, `json`, `tbasic` format. By default, `consoleFormat` is set to `dev`.
...
|This setting specifies the required format for the `messages.log` file. Valid values are `simple`, `json`, or `tbasic` format. By default, `messageFormat` is set to `simple`.
...
|
|This setting controls the format of the trace log. The default format for Liberty is `ENHANCED`. You can also use `BASIC`, `TBASIC`, and `ADVANCED` formats.
