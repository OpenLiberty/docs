...
|This attribute, when set to `true` will merge stack traces emitted to standard-out/standard-error into a single log event.  By default this value is set to `false` and each individual line of the stack trace are emitted as individual log events.
