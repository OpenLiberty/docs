...
The wildcard value `*=audit` applies to all traceable code that is running on the server.
The `:com.myco.mypackage.*=finest` value applies to all classes with the package name that begins with `com.myco.mypackage.`.
...
General trace that including method entry, exit, and return values
...
|fine
|Trace information - General trace that includes method entry, exit, and return values
