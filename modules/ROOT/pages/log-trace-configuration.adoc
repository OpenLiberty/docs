...
When logs are in JSON format, you can use the `jsonAccessLogFields` attribute to replace the default JSON HTTP access log fields with fields that are specified by the `logFormat` attribute of the `accesLogging` element within the xref:reference:config/httpEndpoint.adoc#accessLogging[the `httpEndpoint` element]. With this configuration, you can receive information that is otherwise not available in JSON logs, such as the remote user ID, request headers, and more. These logs can be used by log analysis tools, such as the Elastic stack, to better monitor your server.
...
For more information, see link:/blog/2020/07/30/Using-JSON-with-Open-Liberty-20.0.0.8.html#accessJSON[HTTP access log fields in JSON logs].
