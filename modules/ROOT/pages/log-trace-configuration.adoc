...
For more information on logging levels, see <<#logging_levels,Valid logging levels>>.
...
You can specify the trace specification to selectively enable trace. The logging properties can be set in the server configuration file by adding a logging element to the server configuration file as follows:
...
The following table lists the valid log detail levels that you can specify in the trace specification attribute for application servers at Open Liberty.
An empty value is allowed and treated as 'disable all trace'.
Any component that is not specified is initialized to a default state of `*=info`.
