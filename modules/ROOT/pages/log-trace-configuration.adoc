...
* <<#performance,Performance considerations for logging>>
...
=== High-volume System.out and System.err usage

By default, messages sent to https://docs.oracle.com/javase/8/docs/api/java/lang/System.html#out[System.out] and https://docs.oracle.com/javase/8/docs/api/java/lang/System.html#err[System.err] are sent to both `messages.log` and process stdout/stderr (for example, `console.log`) because the `copySystemStreams` attribute defaults to `true`. With high-volume `System.out`/`System.err` usage, this duplicated output can have a significant performance impact.

You can disable sending `System.out`/`System.err` to process stdout/stderr by setting `com.ibm.ws.logging.copy.system.streams=false` in the `bootstrap.properties` file, as shown in the following example:
...
If `System.out`/`System.err` logging is excessive and causes a performance problem even when sending just to `messages.log`, you can disable them temporarily with the following configuration in your `server.xml` file:

[source,xml]
----
<logging traceSpecification="*=info:SystemOut=off:SystemErr=off"/>
----

However, this approach generally isn't recommended long-term because some components in the Java Class Library and other libraries might have critical diagnostics that write to `System.out` or `System.err`. Instead, it's best to fix such excessive `System.out`/`System.err` usage in your application code.
...
[subs=+quotes]
----
<time stamp> <thread ID> <fully qualified name of logger> <log level attribute> <message ID>: <message>
----
+
...
[subs=+quotes]
----
<time stamp> <thread ID> <class name of logger> <log level attribute> [<class name>] [<method name>] <message>
----
- The <message> will contain the message ID if available. This will be the case for logs emitted to the console and messages log, but the traces log include more specific logs that don't require message IDs.
- The _class name of logger_ will be restricted to 13 characters long.
- The fields in brackets (i.e., class name and method name) will only be available if present. It is common that these fields may not be present in a trace. A scenario where these fields will be populated is if an application provides the `class name` and `method name` parameters to the `Logger` when instrumenting logging statements.

The following example shows a message in `TBASIC` format without the class name and method name fields filled. Notice how `FeatureManager` has been cut to `FeatureManage` (i.e., 13 characters):

[source,console]
----
[2025-05-21, 17:35:56:269 EDT] 0000003d FeatureManage A   CWWKF0011I: The driver server is ready to run a smarter planet. The driver server started in 8.962 seconds.
----

The following is an example of a `TBASIC` formatted message with the class name and method name fields provided. Notice in this example how the _logger_ name is `webcontainer`, but the class the message originates from is `com.ibm.ws.webcontainer.osgi.DynamicVirtualHost`:

[source,console]
----
[2025-05-21, 17:57:28:064 EDT] 00000041 webcontainer  I com.ibm.ws.webcontainer.osgi.DynamicVirtualHost addWebApplication SRVE0250I: Web Module ibm/api has been bound to default_host.
----


ENHANCED (trace log)::
Messages that are in `ENHANCED` format use the following pattern:
+
[subs=+quotes]
----
<time stamp> <thread ID> <object ID> <fully qualified class name of logger> <log level attribute> [<method name>] <message>
----
- The <message> will contain the message ID if available. The traces log contains more component specific logs that don't require message IDs.
- The fields in brackets (i.e., method name) will only be available if present. It is common that this field may not be present in a trace. A scenario where this field will be populated is if an application provides the `method name` parameter to the `Logger` when instrumenting logging statements.
- The `objectID` field represents the object where the log originates from. This field is always present even if it does not contain a value (i.e., 00000000).
...
[source,console]
----
[2025-05-21, 16:04:36:874 EDT] 0000004c id=f430e04e cs.internal.monitor.computed.internal.ComputedMonitorMetrics > getMonitorMetricID Entry
----

The objectID here is `f430e04e`. This reflects the object ID (i.e., hash) of the `ComputedMonitorMetrics` class.

The following example shows a message in `ENHANCED` format with an "empty" object ID:
For example:

[source,console]
----
[2025-05-21, 15:48:16:213 EDT] 00000001 id=00000000 com.ibm.ws.kernel.launch.internal.FrameworkManager           I CWWKE0002I: The kernel started after 1.954 seconds
----


ADVANCED (trace log)::

Messages that are in `ADVANCED` format use the following pattern:

```
<time stamp> <thread ID> <log level attribute> UOW=<correlation ID> source=<fully qualified class name of logger> [class=<class name>] [method=<method name>] [id=<object ID>] org=<organization> prod=<product> component=<component> [thread=<thread name>] <message>
```
- The <message> will contain the message ID if available. The traces log contains more component specific logs that don't require message IDs.
- The fields in brackets (i.e., class, method name, object ID and thread) will only be available if present. It is common that these field may not be present in a trace.
- In comparison to `ENHANCED` format, the objectID will be omitted instead of having a `00000000` if the trace does not contain an object ID associated to it.
- The fields for organization, product and component are always present even if there is no value assosicated to the field. This results in an empty field (e.g., "org=").
...
In this trace, we see that the method field has a value and indicates the originating method as `setProperty` and the object ID is `5f05cd40`. The organization, product and component are empty, but as they are always present we see `org= prod= component=`.
...
[#trace-log-level]
=== Trace log-level attributes

In addition to message log-level attributes, traces have additional log levels that are listed below. Some traces may have _message log-level attributes_ defined above and some may contain the below _trace log-level attributes_.
...
|`1`
|FINE
|Traces that are at the `FINE granularity.

|`2`
|FINER
|Traces that are at the `FINER granularity.

|`3`
|FINEST
|Traces that are at the `FINEST` granularity.

|`>`
|ENTRY
|Traces that indicate an entry into a method. This on the same granularity as `F

|`<`
|EXIT
|Traces that indicate an exit of a method. This on the same granularity as `FINEST`.
...
[NOTE]
====
If the Logstash Collector or MicroProfile Telemetry features are enabled, the `logFormat` attribute that is configured for those features takes precedence over the `jsonAccessLogFields` configuration. The resulting JSON access logs follow the format that is configured by those features rather than the fields specified in `jsonAccessLogFields`.
====
...
|
|If this setting is set to `true`, messages that are written to the System.out and System.err streams are copied to process `stdout` and `stderr` streams and so appear in the `console.log` file. If this setting is set to `false`, those messages are written to configured logs such as the `messages.log` file or `trace.log` file, but they are not copied to `stdout` and `stderr` and do not appear in `console.log`. The default value is true. With high-volume System.out/System.err usage, setting this to `false` can improve performance by avoiding duplicated output. For more information, see <<#performance,Performance considerations for logging>>.
...
|com.ibm.ws.logging.throttle.max.messages.per.window
|WLP_LOGGING_THROTTLE_MAX_MESSAGES_PER_WINDOW
|This setting specifies the maximum number of log messages permitted over a 5 minute duration. If this limit is exceeded within the current window, any additional messages will be suppressed until the count falls below the threshold. Valid values are any positive integer. The default is `1000`. Set the attribute to `0` to disable throttling.

|throttleType
|com.ibm.ws.logging.throttle.type
|WLP_LOGGING_THROTTLE_TYPE
...
Liberty can limit the number of times repeated log entries can be logged over a five minute period. Repeated log entries are identified by either their messageID's or the entire message content. This feature is enabled by default with a limit of 1000 messages over a 5 minute period. The maximum number of messages can be configured, but the duration cannot.

Each log event's occurrence is tracked. When the threshold is met, logs are suppressed and a warning is logged that the warning printed is `TRAS3016W: The logs are being throttled due to high volume`. The occurrence continues to be tracked even while logs are suppressed, and log events are not logged until the occurrence drops below the configured threshold. There are a limited number of keys that are tracked, so regular garbage collection occurs to clean low occurring logs.

==== Configuration

You can configure log throttling based on the full message or messageID by using the `throttleType` logging attribute. The number of messages before throttling occurs can be configured using the `throttleMaxMessagesPerWindow` attribute. Log throttling is disabled by setting `throttleMaxMessagesPerWindow` to 0.
