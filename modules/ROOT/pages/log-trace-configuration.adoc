...
- `console.log` - This file is created by the `server start` command. It contains the redirected standard output and standard error streams from the JVM process. This console output is formatted for human readability and it lacks some information that is useful for automated log analysis. The console.log file is reset when the server is restarted.
...
The `console.log` file is created by redirecting the process `stdout` and `stderr` streams to a file. As a result, Open Liberty is unable to offer the same level of management, like log rollover, as it offers for the `messages.log` file. If you are concerned about the size of the `console.log` file, you can direct server logging messages to the `messages.log` file instead. All the server logging messages that are sent to the `console.log` file are written to the `messages.log` file, and you can configure file rollover in the messages log. However, when you set the console log level to `OFF`, messages from the underlying JVM and any messages that are written to the native  `stderr` or `stdout` streams are still written to the `console.log` file.


To redirect server logging messages from the console log to the messages log and configure the `messages.log` file to roll over three times at 100 Mb, use the following configuration:
...
Liberty has a high-performance binary log format option that reduces the resources that are needed to write trace files. Generally, when you configure binary logging, the `console.log` is set to `OFF` for best performance and logging messages are written to the messages log instead. You can configure binary logging in the `bootstrap.properties` file, as shown in the following example:
...
com.ibm.ws.logging.copy.system.streams=false
----
However, when you set the console log level to `OFF`, messages from the underlying JVM other other data this is written to the native  `stderr` or `stdout` streams are still written to the `console.log` file.
