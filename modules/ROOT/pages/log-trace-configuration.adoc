...
You can enable time-based periodic rollover of Liberty messages and trace logs by specificing a log rollover start time and a log rollover interval duration. The specified rollover start time is your desired time of day when logs should first roll over. The rollover interval duration is the time interval in between consecutive log rollovers.

For example, a server with a rollover start time of midnight and a rollover interval of 1 day will roll over its messages and trace logs once every day, at midnight.

You can configure rollover for Open Liberty messages and trace logs by either editing the `bootstrap.properties` file, specifying environment variables, or using server configuration attributes. The following examples show the configuration for each of these options, with a log rollover start time of midnight and a log rollover interval of 1 day:

* Configure time-based log rollover in the `bootstrap.properties` file:
+
[source,properties,linenums,role="code_column"]
----
...
----
+
* Configure time-based log rollover with environment variables in the `server.env` file:
+
[source,properties,linenums,role="code_column"]
----
...
----
+
* Configure time-based log rollover with server configuration attributes in the `server.xml` file:
+
[source,properties,linenums,role="code_column"]
----
