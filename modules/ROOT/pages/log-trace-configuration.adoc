...
Messages that are in SIMPLE format include one of the following message log-level attributes directly before the message ID code:

- `A`: This attribute indicates the `AUDIT` log level. Audit messages are written to the system output stream.
- `E`: This attribute indicates the `ERROR` log level. Error messages are written to the system error stream.
- `I`: This attribute indicates the `INFO` log level. Info messages are written to the system output stream.
- `W`: This attribute indicates the `WARNING` log level. Warning messages are written to the system output stream.
- `O`: This attribute does not specify a log level but indicates that the message is written to the system output stream.
- `R`: This attribute does not specify a log level but indicates that the message is written to the system error stream.

For more information, see the descriptions for the consoleLogLevel and copySystemStreams attributes for the config:logging[] element.
...
In this example, the `A` attribute that precedes the `CWWKF0011I` message ID code indicates that the message log level is `AUDIT`.
Messages that are in `TBASIC` format include the same message log-level attributes that are listed in the description of the `SIMPLE` format.
