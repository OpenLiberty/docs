...
Each log event's occurrence is tracked. When the threshold is met, logs are suppressed and a warning is logged. The occurrence continues to be tracked even while logs are suppressed, and log events are not logged until the occurrence drops below the configured threshold. There are a limited number of keys that are tracked, so regular garbage collection occurs to clean low occurring logs.
