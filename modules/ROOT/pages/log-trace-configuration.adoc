...
For more information, see **link pending**
...
|When logs are in JSON format, you can use the jsonAccessLogFields attribute to replace the default JSON HTTP access log fields with fields that are specified by the logFormat property of the accesLogging attribute for the httpEndpoint element.
