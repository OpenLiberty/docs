...
You can set logging properties in the server configuration file by adding a logging element to the server configuration file as follows:
...
where `<component>` is the component for which to set a log detail level, and `<level>` is one of the valid logger levels like `off`, `fatal`, `severe`, `warning`, `audit`, `info`, `config`, `detail`, `fine`, `finer`, `finest` or `all`.
Separate multiple log detail level specifications with colons `:`.
For a given component, the level is determined by the most specific trace specification that applies to that component.

Components correspond to Java packages and classes, or to collections of Java packages.
Use an asterisk `*` as a wildcard to indicate components that include all the classes in all the packages that are contained by the specified component. For example:
...
The following table lists the valid levels for application servers at Open Liberty.
...
|finest
|Trace information - A more detailed trace that includes all the detail that is needed to debug problems
