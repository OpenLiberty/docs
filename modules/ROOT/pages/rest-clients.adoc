...
REST clients can be implemented either synchronously or asynchronously. A synchronous client constructs an HTTP structure, sends a request, and waits for a response. An asynchronous client constructs an HTTP structure, sends a request, and moves on. In this case, the client is notified when the response arrives and the original thread, or another thread, can then process the response. The return type of the interface method determines whether a RESTful service is invoked synchronously or asynchronously. Before MicroProfile Rest Client 2.0, if the method has a `CompletionStage` return type, the service is invoked asynchronously and all other return types are invoked synchronously. With MicroProfile Rest Client 2.0, a method with the `Publisher` return type is also invoked asynchronously. One advantage of asynchronous behavior is that multiple services can be invoked simultaneously, resulting in faster overall execution.
