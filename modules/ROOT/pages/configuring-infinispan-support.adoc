...
After you configure embedded mode in the `server.xml` file, create an `infinispan.xml` file in the `server.xml` file to enable the link:https://infinispan.org/docs/11.0.x/titles/embedding/embedding.html#setting_up_clusters[JGroups Kubernetes transport stack]:
[source,xml]
----
<infinispan>
  <jgroups>
