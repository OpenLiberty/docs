...
Starting with version 26.0.0.7, the `trackLoggedOutSSOCookies` attribute is enabled by default. To track the cookies that are logged out on a particular Liberty server, you can configure it in your `server.xml` file.
