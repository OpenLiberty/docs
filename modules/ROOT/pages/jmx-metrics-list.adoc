...
The unit for this attribute is a minute since the epoch began, which is 00:00:00 Coordinated Universal Time on 1 January 1970.
This minute can be the current minute.
...
|`MinutePrevious`
|Minute
|The second-most recent minute, which is the minute before the `MinuteLatest` attribute, in which an update was made to this MXBean.
The unit for this attribute is a minute since the epoch began, which is 00:00:00 Coordinated Universal Time on 1 January 1970.

|`MinutePreviousMaximumDuration`
|Nanoseconds
|The longest recorded `ResponseTime` attribute within the second-most recent minute, which is the minute before the `MinuteLatest` attribute, in which an update was made to this MXBean.
For this attribute, being within a minute is defined as 0:00:00 to 0:59:999.

|`MinutePreviousMinimumDuration`
|Nanoseconds
|The shortest recorded `ResponseTime` attribute within the second-most recent minute, which is the minute before the `MinuteLatest` attribute, in which an update was made to this MXBean.
For this attribute, being within a minute is defined as 0:00:00 to 0:59:999.
