...
The object name of the MXBean for these attributes is `WebSphere:type=McpOperationStatistics,name="mcpMethod:<mcpMethod>;genAiTool:<genAiTool>;errorType:<errorType>;genAiPrompt:<genAiPrompt>;rpcStatus:<rpcStatus>;genAiOp:<genAiOp>;jsonrpcVer:<jsonrpcVer>;mcpVer:<mcpVer>;netProto:<netProto>;netProtoVer:<netProtoVer>;netTransport:<netTransport>;resourceUri:<resourceUri>"`:
...
Performance data is available for each unique combination of session attributes.
Each unique combination has its own MXBean.

The following attributes are available for the `McpSessionStatistics` MXBean.
The object name of the MXBean for these attributes is `WebSphere:type=McpSessionStatistics,name="session;errorType:<errorType>;jsonrpcVer:<jsonrpcVer>;mcpVer:<mcpVer>;netProto:<netProto>;netProtoVer:<netProtoVer>;netTransport:<netTransport>"`:
