...
Enable the feature:monitor[display=Performance Monitoring] feature to monitor JMX metrics for your Open Liberty runtime environment.
...
* <<mcp-stats,MCP server monitoring>>
...
|`getTotalInboundResponses(appName, "604");`
|Application
|The number of inbound 604 (Does Not Exist Anywhere) responses that belong to each application.
...
|`getTotalOutboundResponses(appName, "604");`
|Application
|The number of outbound 604 (Does Not Exist Anywhere) responses that belong to each application.
...
You can use the `McpOperationStatistics` and `McpSessionStatistics` MXBeans to monitor MCP server operations and sessions in Open Liberty.
Both MXBeans activate automatically when feature:mcpServer-1.0[display=MCP Server 1.0] and feature:monitor[display=Performance Monitoring] are enabled.
The monitoring component filter value is `MCP`.
MXBean instances are created dynamically as operations and sessions are observed, and are removed when the associated application is unloaded.

[#mcp-operation-stats]
=== MCP operation monitoring: McpOperationStatistics MXBean
You can use the `McpOperationStatistics` MXBean to monitor MCP operation performance data in Open Liberty.
Performance data is available for each unique combination of MCP method and attributes.
Each unique combination has its own MXBean.

The following attributes are available for the `McpOperationStatistics` MXBean.
The object name of the MXBean for these attributes is `WebSphere:type=McpOperationStatistics,mcpMethod=<mcpMethod>,genAiTool=<genAiTool>,errorType=<errorType>,genAiPrompt=<genAiPrompt>,rpcStatus=<rpcStatus>,genAiOp=<genAiOp>,jsonrpcVer=<jsonrpcVer>,mcpVer=<mcpVer>,netProto=<netProto>,netProtoVer=<netProtoVer>,netTransport=<netTransport>,resourceUri=<resourceUri>`:

.McpOperationStatistics MXBean attributes
[%header,cols="6,3,12"]
|===
...
|`McpMethodName`
|Not applicable
|The MCP JSON-RPC method name, for example, `tools/call`, `initialize`, `tools/list`, or `ping`. Always present.
...
|`GenAiPromptName`
|Not applicable
|The name of the prompt for prompt operations. `null` if not applicable.

|`GenAiOperationName`
|Not applicable
|The generative AI operation name. `null` if not applicable.
...
|Not applicable
|The JSON-RPC protocol version, for example, `2.0`.

|`McpProtocolVersion`
|Not applicable
|The MCP protocol version negotiated with the client, for example, `2025-11-25`.

|`NetworkProtocolName`
|Not applicable
|The network protocol name, for example, `HTTP`.

|`NetworkProtocolVersion`
|Not applicable
|The network protocol version, for example, `1.1`.
...
|`CountDetails`
|Not applicable
|Details about the `Count` attribute value, including the last update time stamp.
...
|`DurationDetails`
|Nanoseconds
|Details about the `Duration` attribute value, including minimum, maximum, and total values.

|===

[#mcp-session-stats]
=== MCP session monitoring: McpSessionStatistics MXBean
You can use the `McpSessionStatistics` MXBean to monitor MCP session performance data in Open Liberty.
Performance data is available for each unique combination of MCP session attributes.
Each unique combination has its own MXBean.

The following attributes are available for the `McpSessionStatistics` MXBean.
The object name of the MXBean for these attributes is `WebSphere:type=McpSessionStatistics,session=true,errorType=<errorType>,jsonrpcVer=<jsonrpcVer>,mcpVer=<mcpVer>,netProto=<netProto>,netProtoVer=<netProtoVer>,netTransport=<netTransport>`:

.McpSessionStatistics MXBean attributes
[%header,cols="6,3,12"]
|===
...
|`JsonrpcProtocolVersion`
|Not applicable
|The JSON-RPC protocol version used when the session was created, for example, `2.0`.

|`McpProtocolVersion`
|Not applicable
|The MCP protocol version negotiated during `initialize`, for example, `2025-11-25`.
