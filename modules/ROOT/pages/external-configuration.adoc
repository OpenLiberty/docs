...
A higher ordinal means that the values taken from this https://javadoc.io/static/org.eclipse.microprofile/microprofile/4.0.1/index.html?overview-summary.html[config source] override config sources with a lower ordinal value.
The following table describes the default config sources in MicroProfile Config and their corresponding values.

[cols="1,1,2"]
|===
|Config Source |Default ordinal values |Example
