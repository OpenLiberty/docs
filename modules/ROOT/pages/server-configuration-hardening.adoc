...
* Basic and Quick Start Security user registries including in-memory identity stores are useful for development environments, but they are not acceptable for readiness testing, quality assurance, or production use. For production, use an LDAP user registry, federated registries, or create a custom user registry.
+
...
+
* In‑memory identity stores are disabled by default. You must explicitly enable them before you can use them.
...
Currently, Open Liberty supports AES-128 encryption and AES-256 encryption.
For more information, see xref:password-encryption.adoc[Password encryption limitations].

With AES encryption, an encryption key must configured for decryption.
The encryption key can be configured by setting either the `wlp.password.encryption.key` or `wlp.aes.encryption.key` property.
...
This separate configuration file must contain only a single property declaration, and must be stored outside the Liberty installation directory.
For more information on setting `wlp.aes.encryption.key` property, see link:https://openliberty.io/docs/latest/bring-your-own-aes-256-key.html[Bring your own AES-256 key for Liberty passwords].
