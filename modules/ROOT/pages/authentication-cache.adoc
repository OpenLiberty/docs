...
Because the creation of a subject might affect performance, Open Liberty provides an authentication cache to store a subject after an authentication of a user is successful. You can also configure a distributed authentication cache that is shared among a group of Open Liberty servers that usey the same set of users and groups by using JCache.
...
Consider the following points when you configure the authentication cache.
