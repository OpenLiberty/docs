...
For example, [Apache Johnzon](https://johnzon.apache.org/) is a third-party JSON-B implementation. 
...
You can also mix and match reference implementations and third-party implementations, as long as both implementations support the same Jakarta EE version. 

For example, Apache Johnzon also provides a JSON-P implementation. 
In this example, a server is configured to use Johnzon instead of the default reference implementation for JSON-P, which is Glassfish. However, the server continues to use Yasson for JSON-B:
...
=== Avoiding trouble
Other features, such as feature:microProfile[display=MicroProfile], that enable the JSON-P and JSON-B features by default.
However, you can still still provide your own third-party implementation for the JSON-P or JSON-B API, even when the reference implementations are enabled by default.
The` jsonbContainer` and `jsonpContainer` features supersede the` jsonb` and `jsonp` features. Enabling either feature allows you to provide a third-party implementation.

If logging is enabled, the following information message confirms this behavior.
...
However, you still must configure a `bell` element that specifies the JSON API implementation that your application uses. If not, the following error occurs when your application attempts to use the JSON API. 
