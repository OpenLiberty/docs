...
InstantOn uses the Checkpoint/Restore In Userspace (link:https://criu.org/[CRIU]) feature of the Linux kernel to take a checkpoint of the JVM that can be restored later. With InstantOn, the application is developed as normal, and then InstantOn makes a checkpoint of the application process when the application container image is built. When the application is restored, it runs in the same JVM, which provides complete parity between development and production. Because the CRIU checkpoint process takes only seconds, your CI/CD process is barely affected.
...
The following sections describe the prequisites and processes to build and run an InstantOn application image:
...
To use Docker, a version 23.0 or greater is required. Earlier versions of Docker did not support the `CHECKPOINT_RESTORE` Linux capability, which prevents CRIU from doing a checkpoint or restore of a process in container.
...
You can use these steps with either Podman and Docker to build an Instanton application image. For Docker, version 23.0 or later is required. The following examples assume you are using Docker to build an application iamge that is named `liberty-app`.
...
After you build an InstantOn application container image, you can verify a successful restore by setting this environment variable to run locally. For example, by using the following Podman command:
...
Other public cloud Kubernetes services might also work if they have the <<#required-to-restore,prerequisites>> to allow CRIU to restore the InstantOn application process.
...
The Open Liberty public features enabled by the feature:webProfile[display=Jakarta EE Web Profile] and feature:microProfile[display=MicroProfile] features can be enabled individually, depending on the needs of the application. This option avoids enabling the complete set of features that are enabled by the convenience features. 

In addition to the features that are enabled in the Jakarta EE Web Profile and MicroProfile convenience features, InstantON also supports the following features:
