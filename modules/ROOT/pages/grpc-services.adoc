...
=== gRPC Clients
The gRPC Client feature enables web applications to consume gRPC services. It provides access to a link:https://netty.io[Netty] gRPC client and the related libraries. A web application must provide a client implementation and stubs, and can make outbound calls with a link:https://grpc.github.io/grpc-java/javadoc/io/grpc/class-use/ManagedChannel.html[io.grpc.ManagedChannel] class, without needing to provide the supporting client libraries.
