...
When a user logs out from an application that is protected by LTPA, the LTPA token value is destroyed on the client side. However, the token value might be manually copied before logout, then manually added to subsequent browser requests in an attempt to gain unauthorized access to the application after logout. 
