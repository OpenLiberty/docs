...
[source,sh]
----
cosign verify-blob --insecure-ignore-tlog --key WebSphereLiberty_02-13-2023.pem --signature openliberty-23.0.0.2.zip.cosign.sig openliberty-23.0.0.2.zip
----

This example uses `cosign` version 1.x to verify the  `WebSphereLiberty_02-13-2023.pem` public key file and `openliberty-23.0.0.2.zip.cosign.sig` signature file to verify the `openliberty-23.0.0.2.zip` release package.
Replace the signature file and package version values according to the package that you want to verify. If you are using `cosign` version 2.x, add the `--insecure-ignore-tlog` flag to the command.
