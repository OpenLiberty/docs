...
In the following diagram, a client requests access to protected resources in an application. During authentication, the client request is routed to an Open ID Connect (OIDC) identity provider, which confirms the client's identity and security roles. The OIDC identity provider sends authentication data back to the application, where it is used to authorize client access to protected resources. During xref:authorization.adoc[authorization], Open Liberty checks the roles that are specified for the client against the roles that are defined for the application resources. The client is granted access to any resources that match their role.
...
- xref:single-sign-on.adoc#_openid_connect_oidc[OpenID Connect (OIDC)]
