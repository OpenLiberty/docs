...
Some existing configurations might need to access global libraries, which can be used by any Jakarta EE application. Global libraries are kept in a global library directory and can be referenced from the class loader configuration for each Jakarta EE application. However, other kinds of applications, such as OSGi applications, cannot use global libraries.
