...
The virtual host configuration uses wildcard entries with the ports for its virtual host entries. You can use wildcard entries for aliases by port to specify that all valid hostname and address combinations on a particular port map to a particular virtual host. You can specify any number of aliases for a virtual host. The default alias is `*:80`, which uses an external port that is not secure.
