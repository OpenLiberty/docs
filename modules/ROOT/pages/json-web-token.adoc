...
During JWT signature verification, JWKS documents are used to identify the appropriate public key for validation. Open Liberty matches parameters in the JWT header against the corresponding fields in the key entries within a JWKS document.

Open Liberty performs key selection using one, and only one, of the following JWT headers, in this priority order:
...
For example, if a JWT contains an `x5t` header parameter but lacks both `kid` and `x5t#S256`, Liberty looks for a key in the JWKS document with a matching `x5t` value to verify the signature. If a matching value is not found, signature verification fails.

Similarly, if a JWT contains both `kid` and `x5t#S256` header parameters, Liberty expects to find a key in the JWKS document with a matching `kid` value. If the `kid` does not match, signature verification fails; Liberty does not fall back to the `x5t#S256` value to look up a key.
