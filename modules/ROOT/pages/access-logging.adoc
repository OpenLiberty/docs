...
You can enable time-based periodic rollover of your HTTP access log file by specifying a log rollover start time and a log rollover interval duration. The specified rollover start time is your desired time of day when the HTTP access log file should first roll over. The rollover interval duration is the time interval in between consecutive HTTP access log file rollovers.

For example, a server with a rollover start time of midnight and a rollover interval of 1 day will roll over its HTTP access log once every day, at midnight.

Enable time-based rollover of your HTTP access log file by using the `rolloverStartTime` and `rolloverInterval` attributes of the `httpAcccessLogging` or `accessLogging` elements in the `server.xml` file. The following table lists the two attributes, and their respective descriptions and permitted values.
...
The following examples have time-based periodic rollover enabled for the HTTP access log file in the `httpAcccessLogging` and `accessLogging` elements, with `rolloverStartTime` set to  midnight and `rolloverInterval` set to 1 day:
