...
The Oracle driver tracing will be combined with Liberty tracing by default since both utilize the `java.util.logging` library. As of version 23.0.0.2, Liberty has added support for the seperation of Oracle trace from Liberty trace. This will allow users to provide standalone Oracle JDBC trace when requested by Oracle support.

The following `server.xml` file example shows you how to configure the Oracle debugging driver.  No trace specification is required.
...
----
-Doracle.jdbc.Trace=true
-DoracleLogFileName=<desired-log-name>.log
-DoracleLogPackageName=<desired-package-to-trace>
----
...
Note: `oracleLogFileSizeLimit=0` means there is no limit and the file can grow to any size. 

For updated settings recommended by Oracle please visit their https://docs.oracle.com/en/database/oracle/oracle-database/21/jjdbc/JDBC-diagnosability.html#GUID-5E8CB599-C7D8-48E7-87E7-53559D22D318[support page].
