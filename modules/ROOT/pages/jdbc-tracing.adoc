...
By default, the DB2 JCC driver tracing is combined with Liberty tracing because the DB2 logwriter provided by Liberty routes trace to the same file. However, Liberty also supports separating DB2 JCC trace from Liberty trace. You can use this function to provide a stand-alone DB2 JCC JDBC trace to DB2 support, if needed.

This cusomized tracing can be set on a per DataSource basis, such as the example below:
...
- `traceLevel` : This property specifies the level of trace.
- `traceDirectory` : This property specifies the directory to into which trace is written.
- `traceFile` : This property specifies the name of the file into which trace is written.
- `traceFileAppend` : This property specifies wheather to append to (true) or overwrite (false) the `traceFile`.
- `traceOption` (Optional) : This property when set to `0` specifies that a single trace file will be generated, and that no limit to the size of the file exists. 

For additional information about these properties and additional configuration settings, such as `traceFileSize` and `traceFileCount`, see https://www.ibm.com/docs/en/db2/11.5?topic=information-data-server-driver-jdbc-sqlj-configuration-properties(IBM Data Server Driver for JDBC and SQLJ configuration properties
)
