...
With the xref:reference:feature/concurrent-1.0.adoc[Concurrency Utilities for Java EE feature], Liberty provides a robust, fully spec-compliant implementation of Concurrency Utilities for Java EE. MicroProfile Context Propagation is included with the xref:reference:feature/mpContextPropagation-1.0.adoc[MicroProfile Context Propagation feature]. Both features can be enabled and used in tandem. Try it out today, and start enjoying the advantages of concurrency and task scheduling in a Java EE environment.
