...
|Specifies the number of seconds since the JWT token was issued. The `iat` claim must be present in the JWT and the configured number of seconds since `iat` should not have elapsed. If the configured number of seconds since `iat` has elapsed, then the request is rejected with an unauthorized (401) response. This property is available in MicroProfile JSON Web Token 2.1 and later.
|On each request to a protected resource.
...
The following MicroProfile Config properties can be specified when MicroProfile Telemetry is enabled. For more information, see the https://download.eclipse.org/microprofile/microprofile-telemetry-1.0-RC1/tracing/microprofile-telemetry-tracing-spec-1.0-RC1.html[MicroProfile Telemetry specification].
...
|Enables tracing by MicroProfile Telemetry. The default is `false`.
|Application startup
