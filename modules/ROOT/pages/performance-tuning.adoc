...
By default, messages sent to https://docs.oracle.com/javase/8/docs/api/java/lang/System.html#out[System.out] and https://docs.oracle.com/javase/8/docs/api/java/lang/System.html#err[System.err] are sent to both `messages.log` and process stdout/stderr (for example, `console.log`) because xref:log-trace-configuration.adoc[copySystemStreams / com.ibm.ws.logging.copy.system.streams] default to `true`. With high volume `System.out`/`System.err` usage, this duplicated output might have a significant performance impact. You can disable sending `System.out`/`System.err` to process stdout/stderr by setting `com.ibm.ws.logging.copy.system.streams=false` in `bootstrap.properties`.

If `System.out`/`System.err` logging is excessive and causes a performance problem even when sending just to `messages.log`, then you can disable them temporarily with `<logging traceSpecification="*=info:SystemOut=off:SystemErr=off"/>`. However, this approach generally isn't recommended long-term because some components in the Java Class Library (and others) might have critical diagnostics that write to `System.out` or `System.err`. Instead, it's best to fix such excessive `System.out`/`System.err` usage.
