...
You can tune these parameters and attributes to achieve better performance.
...
You can maximum heap size to 1024m, set a system property, enable verbose output for class loading and enable verbose garbage collection.
...
For more information, see xref:reference:config/server-configuration-overview.html#jvm-options[jav.options]
...
The Jakarta Contexts and Dependency Injection feature might be included in your server configuration because other features include it, such as the feature:microProfile[display=MicroProfile] feature and the feature:webProfile[display=Jakarta EE Web Profile] feature.
