...
For more information, see xref:reference:config/server-configuration-overview.html#jvm-options[jvm.options]
...
When you use `numConnectionsPerThreadLocal`, consider two other values, the number of application threads and the connection pool maximum connections.
...
To set this option, review the application code (or an SQL trace that you gather from the database or database driver) for all unique prepared statements and ensure that the cache size is larger than the number of statements.
