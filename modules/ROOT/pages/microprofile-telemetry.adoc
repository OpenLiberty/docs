...
When your Liberty runtime hosts multiple applications, which is typically associated with virtual machine environments, you might choose to enable OpenTelemetry at the application level to collect logs, metrics, and traces for each application. Although this option does not collect runtime-level telemetry, it gives you fine-grained control of the data you collect for each application.
