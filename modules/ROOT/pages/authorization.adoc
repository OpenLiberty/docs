...
The `name` attribute of the `security-role` element corresponds to a security role that are specified in the application. The `group name` elements correspond to the user groups that are specified by a user registry or SSO provider.
