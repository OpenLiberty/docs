...
[horizontal]
* By: identifies the client facing interface of the proxy
...
Liberty only utilizes the Forwarded “for” (the “for” parameter can optionally include the port), “host” and “proto” parameters, or also X-Forwarded-For, X-Forwarded-Port, X-Forwarded-Host, and X-Forwarded-Proto headers.
