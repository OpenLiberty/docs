...
Jakarta Data Providers supply an implementation of a repository, which can be injected into applications via CDI (Contexts and Dependency Injection). Jakarta Data Providers can be third-party products or can be provided by the application server.

Open Liberty includes a built-in Jakarta Data Provider for relational database access that can be used with the built-in Jakarta Persistence provider, EclipseLink, or with the Hibernate Jakarta Persistence provider.

Alternatively, you can use third-party Jakarta Data providers, such as Hibernate ORM (for relational data) and Eclipse JNoSQL (for various types of non-relational data), with Open Liberty. The JNoSQL Jakarta Persistence extension is not required, as the Open Liberty Jakarta Data provider and JNoSQL Jakarta Data provider can be used simultaneously.
...
An entity can optionally have a static metamodel, which might be generated from the entity class by development tooling. They are also simple enough for a developer to write and understand, with uppercase constants for entity attribute names and lowercase constants representing additional detail about each respective entity attribute. The static metamodel can be used by the repository interface as well as the application in defining and invoking data access operations in a more type-safe manner.

By convention, static metamodel interface classes are named to begin with the underscore (_) character, followed by the entity class name.
...
The repository supertypes offer a variety of pre-defined methods for commonly-used data access operations. The first type parameter to the repository supertypes specifies a primary Entity class that can be assumed for repository methods that do not otherwise specify an Entity class. The second type parameter to the repository supertypes specifies the type of the Id attribute that can be assumed for repository methods that do not otherwise specify an Id type.
