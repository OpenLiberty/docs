...
When the feature:sessionDatabase[display=Database Session Persistence] is enabled in your `server.xml` file, you can reference an externally created table for session persistence by specifying the `httpSessionDatabase` attribute, as shown in the following example.
