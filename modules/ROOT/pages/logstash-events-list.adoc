...
When Open Liberty servers run in Docker containers that you manage, you can set the `CONTAINER_HOST` and `CONTAINER_NAME` environment variables when you start your Docker container. Setting these environment variables ensure that the Logstash collector feature tags the records that it sends with the appropriate host and container name, which aids in problem determination. If you do not set these environment variables, you might find it difficult to determine which container sent which events when you use a dashboard that shows events from multiple containers.
