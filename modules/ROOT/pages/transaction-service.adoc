...
The older alternative to peer recovery in a cloud environment is to route WS-AtomicTransaction traffic through a proxy server using `externalURLPrefix`. This may be done by setting `externalURLPrefix` to the pod IP address; for example:
...
It has been observed in some Kubernetes environments that this may cause intermittent socket timeout exceptions when non-WS-AtomicTransaction traffic communicates through a Kubernetes service to the same destination pod on the same port. To avoid this, consider adding an additional config:httpEndpoint[display=HTTP Endpoint] with a different port solely for `externalURLPrefix` traffic, add an additional `hostAlias` for this port in the config:virtualHost[display=Virtual Host], and configure `externalURLPrefix` to use this new port.
