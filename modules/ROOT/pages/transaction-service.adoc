...
Setting the `acceptHeuristicHazard` attribute to `false` enables last participant support (LPS). LPS allows a single one-phase resource to participate in a two-phase transaction with one or more two-phase resources.
