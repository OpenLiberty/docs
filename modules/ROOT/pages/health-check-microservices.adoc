...
The configuration for defining the update interval is the `checkInterval` attribute of the `<mpHealth>` element when using the `mpHealth-4.0` feature. This configuration is required to enable the file-based health-check mechanism. The optional `startupCheckInterval` configuration value is used during the startup of the feature and is the interval between health-check queries for establishing the UP statuses for all three health check types. The `startupCheckInterval` configuration value defaults to 100ms.

The environment variable configuration for the `checkInterval` attribute is `MP_HEALTH_CHECK_INTERVAL`. The environment variable configuration for the `startupCheckInterval` attribute is `MP_HEALTH_STARTUP_CHECK_INTERVAL`. If both an environment variable configuration and a server.xml configuration are detected, then the server.xml configuration takes precedence.

The following example shows the configuration for 'checkInterval' and 'startupCheckInterval'.
