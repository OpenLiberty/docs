...
By default, in MicroProfile Health versions 2.0 and later, the overall Readiness status is DOWN, with an empty response, until all the deployed applications in Open Liberty are started. In MicroProfile Health versions 3.0 and later, a new MicroProfile Config property, `mp.health.default.readiness.empty.response=UP`, is available. You can specify this property to change the overall default Readiness status to `UP` during application startup, for applications that do not specify any user-defined health checks.
