...
MicroProfile Fault Tolerance provides different annotations that can help you deal with failure.
Combining the `@Asynchronous` Fault Tolerance annotation with the `CompletionStage` interface provides ways for you to write asynchronous code that is resilient to faults.
...
While the `@Asynchronous` annotation can make methods that return a `Future` object run asynchronously, https://download.eclipse.org/microprofile/microprofile-fault-tolerance-2.1/microprofile-fault-tolerance-spec.html#_interactions_when_returning_a_code_future_code[Fault Tolerance can be applied only to asynchronous methods that return a CompletionStage object].
