...
Eclipse offers plug-ins that do a similar analysis on the source.
You can install the following plug-ins from the https://marketplace.eclipse.org/content/ibm-websphere-application-server-migration-toolkit-was-liberty[Eclipse marketplace] by dragging the install button onto your Eclipse title bar:
