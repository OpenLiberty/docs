...
You can test the connection to JDBC, JCA, JMS, and Cloudant resources by using the Server Config tool.

The following features must be enabled on your server in addition to the Admin Center configuration.
...
In the Server Config tool, navigate to the resource for which you want to verify the connection. A **Test** button is present and visible to you.
...
In the selected authentication method, specify the required fields and click **Connection Test**.
