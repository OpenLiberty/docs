...
----
+
. After you access the **Design** tab in the **Server Config** tool, select the `DataSource` element in the configuration to see enabled features and parameter settings. The following screen capture shows the `DataSource` elements in the configuration and the enabled features and parameter settings. In the display window for your chosen `DataSource` element, click the **Test** button. The following screen capture shows the **Design** tab and the selected display window for your chosen `DataSource` element in the **Server Config** tool.
+
image::ui_test_connection_1.png[align="center"]
+
. The **Connection Test** page helps choose the authentication method that you want to employ for the connection test. The three authentication methods are: **Container authentication**, **Application authentication**, or **No resource reference**. Each of these modes can be accessed from a browser tab within the tool. Select any of the authentication methods, specify the required fields and click the **Connection Test** button. The following screen capture shows the **Connection Test** page and the authentication methods.
+
