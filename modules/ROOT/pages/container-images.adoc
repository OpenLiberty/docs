...
Note: When using Docker CLI to pull from link:https://www.ibm.com/cloud/container-registry[IBM Container Registry] (ICR), there is no need to specify the `latest` tag. This is because the CLI will automatically use the `latest` tag by default if no tag is specified. You can use the `docker pull <image_name>` command to pull the latest version of the image from ICR.
