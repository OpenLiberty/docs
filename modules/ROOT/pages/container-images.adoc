...
IMPORTANT: The final Liberty release that will publish container images built on UBI Standard will be `26.0.0.12`. Avoid trouble by using a UBI Minimal based image instead of UBI Standard. For more information, see announcement at https://www.ibm.com/support/pages/node/7268632[IBM Support page].
...
Base image type::
Liberty images are available with two different types of Red Hat Universal Base Image (UBI): UBI Minimal or UBI Standard. 
Image tags based on UBI Minimal end with `-ubi-minimal`. Image tags based on UBI Standard end with `-ubi`. 
