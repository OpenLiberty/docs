...
Metric units are included along with both the metric names and Prometheus names. In MicroProfile 4.0 and earlier, these units can differ because the Prometheus exporter scales metrics to a base unit.
...
|The recent CPU time that is used by the JVM process for all processors that are available to the JVM. The value is between `0` and `1`. This metric is a gauge.
|Base metric
|{base-metric-features}
|{mp-3-0}
...
|The recent average time spent per garbage collection cycle. This metric displays `-1` if the time elapsed in garbage collection or the count is undefined for this collector. This metric is a gauge.
|Base metric
|{base-metric-features}
|{mp-3-0}
