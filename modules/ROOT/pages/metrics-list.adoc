...
{empty} +
In MicroProfile Metrics 5.0, the unit associated to the metric is what is reflected in the Prometheus output. The metrics are not scaled to a base unit. 
...
A separate table for for MicroProfile Metrics 5.0 is included because of changes in the release which affect the formatting of the Prometheus name, base metrics, and the metric types.
The following changes apply to MicroProfile Metrics 5.0:
...
For more information, see xref:mp-50-60-diff.adoc#metrics[Differences between MicrProfile Metrics 5.0 and earlier versions]
...
.MicroProfile Metrics 5.0m metrics reference (`mpMetrics-5.0`)
