...
By default, all monitoring components are enabled. You can configure only a subset of these monitoring components to be reported by specifying the `filter` attribute for the monitor configuration element in your `server.xml` file.If your server is collecting more metrics data than you need, you can improve the server performance by collecting only those metrics that you intend to use. You can identify the relevant monitoring component for each metric by referencing the Metrics that are available for Open Liberty table.
