...
To use the MicroProfile Metrics with the feature:monitor[display=Performance Monitoring] feature, enable the reporting of `vendor` metrics in the `/metrics` endpoint.
...
The _REST base metrics_ introduced in MicroProfile Metrics 2.3 also relies on the Performance Monitoring feature to function.
...
By default, all monitoring components are enabled. You can configure the exact monitoring components to be enabled by configuring the `server.xml` file with the `filter` attribute for the monitor configuration element.

To enable only the monitoring components used by MicroProfile Metrics you can configure the `server.xml` file, as shown in the following example:
