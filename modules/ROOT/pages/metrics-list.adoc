...
When using MicroProfile Metrics with the feature:monitor-1.0[display=Performance Monitoring] feature, you enable the reporting of `vendor` metrics on the `/metrics` endpoint.
Prior to MicroProfile Metrics 2.3, the feature:monitor-1.0[display=Performance Monitoring] feature needs to be explicitly configured.
In MicroProfile Metrics 2.3 and up, the feature:monitor-1.0[display=Performance Monitoring] feature is automatically enabled by the MicroProfile Metrics feature during startup.
It is worth noting that the _REST base metrics_ introduced in MicroProfile Metrics 2.3 also relies on the feature:monitor-1.0[display=Performance Monitoring] feature to function.

The feature:monitor-1.0[display=Performance Monitoring] feature retrieves statistical data from all available monitoring components.

You can identify which MicroProfile Metrics release the associated metrics are offered by identifying the relevant metric in the _Metrics that are available for Open Liberty_ table, and the version it was introduced in.
For example, the Request Timing vendor metrics were introduced in MicroProfile Metrics 2.0.

By default, all monitoring components are enabled. You can configure the exact monitoring components to be enabled by configuring the `server.xml` with the `filter` attribute for the monitor configuration element.

To enable only the monitoring components used by MicroProfile Metrics you can configure the `server.xml`, as shown in the following example:
...
To disable all _vendor metrics_, but to keep the REST _base metrics_ configure the `server.xml`, as shown in the following example:
...
To disable all monitoring components you can configure the `server.xml` like this:
...
|Base metric, no monitoring component associated. Will always be available.
...
This metric is a gauge.
|connectionpool.connectionHandles
...
This metric is a gauge.
|connectionpool.freeConnections
|{vendor-metric-features}
|{mp-2-0}
...
This metric is a gauge.
|connectionPool.inUseTime.total
|{vendor-metric-features}
|{mp-2-0}
...
This metric is a gauge.
|connectionpool.managedConnections
|{vendor-metric-features}
|{mp-2-0}
...
This metric is a counter.
|connectionpool.queuedRequests.total
|{vendor-metric-features}
|{mp-2-0}
...
This metric is a counter.
|connectionPool.usedConnections.total
|{vendor-metric-features}
|{mp-2-0}
...
This metric is a counter.
|Base metric, is available when MP Fault Tolerance feature is enabled.
|{ft-metric-features}
|{mp-ft-3-0}
...
This metric is a counter.
|grpc.client.receivedMessages.total
...
This metric is a gauge.
|grpc.client.responseTime.total
|{grpc-client-metric-features}
|{mp-2-3}
...
This metric is a counter.
|grpc.client.rpcCompleted.total
|{grpc-client-metric-features}
|{mp-2-3}
...
This metric is a counter.
|grpc.client.sentMessages.total
|{grpc-client-metric-features}
|{mp-2-3}
...
This metric is a counter.
|grpc.server.receivedMessages.total
...
|The response time of completed RPCs.
This metric is a gauge.
|grpc.server.responseTime.total
|{grpc-server-metric-features}
|{mp-2-3}
...
This metric is a counter.
|grpc.server.rpcCompleted.total
|{grpc-server-metric-features}
|{mp-2-3}
...
This metric is a counter.
|grpc.server.sentMessages.total
|{grpc-server-metric-features}
|{mp-2-3}
...
This metric is a counter.
|No associated monitoring component, will always be available.
|{vendor-metric-features}
|{mp-2-0}
...
This metric is a counter.
|session.invalidatedbyTimeout.total
|{vendor-metric-features}
|{mp-2-0}
