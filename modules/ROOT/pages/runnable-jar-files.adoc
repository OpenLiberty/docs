...
By default the Open Liberty server will simply run in the JVM created immediately from the command line `java -jar ...` command.  In this case JVM options will be read from the locations specific to this JVM (the command-line and any JVM-defined locations).  In this single JVM case, environment variables will be read from the current shell environment, and Liberty-specific configuration files like `server.env` are not relevant.
