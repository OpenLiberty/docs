...
By default, the Open Liberty server runs in the JVM that is created by running the `java -jar ...` command.  In this case, JVM options are read from the locations that are specific to this JVM: the command-line and any JVM-defined locations.  In this single-JVM case, environment variables are read from the current shell environment, and Liberty-specific configuration files like `server.env` are not relevant.

However, if a `jvm.options` file is detected in any of the xref:reference:config/server-configuration-overview.adoc[locations] that Liberty searches, then a second JVM is launched to run the Liberty server with the specified  options. In this case, a `server.env` file can provide the environment for the second server JVM.

A second JVM is likewise used if the `WLP_JAR_DEBUG` environment variable is set, as described in the following section.
