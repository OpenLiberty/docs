...
For a more secure configuration do not set the `wlp.password.encryption.key` property in the `server.xml` file that stores the password.
...
This separate configuration file must be stored outside the normal configuration directory for the server and must contain only a single property declaration, as shown in the following example.
