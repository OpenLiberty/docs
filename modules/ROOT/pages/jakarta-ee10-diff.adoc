...
=== New Expression Language Resolvers and import changes

Expression Language Resolvers are updated to improve handling of imports and unresolved variables. The `ScopedAttributeELResolver` class is refactored to create an https://jakarta.ee/specifications/pages/3.1/apidocs/jakarta.servlet.jsp/jakarta/servlet/jsp/el/importelresolver[ImportELResolver] class. This new import resolver allows any class or static imports from the `import` attribute to be available to the expression language environment. Previously, imports were only available to the scripting environment. If an expression used an import class, it would resolve to an empty string. The other new resolver is the https://jakarta.ee/specifications/pages/3.1/apidocs/jakarta.servlet.jsp/jakarta/servlet/jsp/el/notfoundelresolver[NotFoundELResolver] class, which always resolves a variable when the other ones do not.

Lastly, default and implicit imports for the scripting environment now also apply to the expression language environment. These default imports are `java.lang.\*`, `jakarta.servlet.*`, `jakarta.servlet.jsp.\*`, and `jakarta.servlet.http.*`.
