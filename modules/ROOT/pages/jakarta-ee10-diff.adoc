...
When you enable enabling the feature:jwt-1.0[display=JSON Web Token
1.0] (jwt-1.0) Liberty feature with Jakarta EE 10 Liberty features, the Servlet APIs are no longer available by default. For Open Liberty applications that require Servlet APIs, you must add the feature:servlet-6.0[display=Jakarta Servlet 6.0] (servlet-6.0) feature to your `server.xml` file.
