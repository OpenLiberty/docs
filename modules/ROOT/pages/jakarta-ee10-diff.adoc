...
=== The bean-discovery-mode attribute  

In previous versions of CDI, a `beans.xml` file, whether empty or non-empty, that does not specify the `bean-discovery-mode` attribute is treated as if the attribute is set to bean-discovery-mode="all" in the file. In CDI 4.0, such a file is treated as if the attribute is set to bean-discovery-mode="annotated" attribute in the file.

For an empty `beans.xml`, you can set the `emptyBeansXmlCDI3Compatibility` attribute to enable compatibility with previous versions. If you set this attribute to true in CDI 4.0, any archives that contain an empty `beans.xml` file are treated as explicit beans archives.

The best practice is to always specify a version for all non-empty `beans.xml` files, as shown in the following example.
