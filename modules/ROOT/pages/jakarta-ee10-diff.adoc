...
Starting with Jakarta EE 10, the Servlet APIs are no longer available by default when you enable the RESTful Web Services (JAXRS) and RESTful Web Services Client feaures. For Open Liberty applications that require Concurrency APIs, you must also add the `servlet-6.0` feature to the `server.xml` file.
