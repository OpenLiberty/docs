...
In previous versions, support for sending and receiving multipart/form-data parts was provided by Liberty-specific APIs. This support is now part of the core Restful Web Services specification and the Liberty-specific APIs are no longer required. Liberty continues to support these APIs, though they are scheduled for deprecation in a future release.
...
The specification was updated to clarify that in the absence of `@Produces` or `@Consumes` annotations, support for any media type is assumed.
