...
In Jakarta Servlet 6.0, you can set attributes on a response cookie either a deployment descriptor file, programmatically with the `jakarta.servlet.ServletContainerInitializer` API, or at request time.
...
You can also set cookie attributes dynamically from an application during pplication startup by using the `jakarta.servlet.ServletContainerInitializer`, as shown in the following example.
...
If conflicts arise among different sources for the cookie configuration, the following precedence order applies, from lowest to highest precedence, to resolve the conflict.
