...
You can skip the verification of encoded characters by setting the `skipEncodedCharVerification` property `true`. The default value for this attribute is `false`.
...
Alternatively, you can set it as a `webContainer` property in the `server.xml` to skip the verification for all deployed applications.

[source,xml]
----
<webContainerskipEncodedCharVerification="true"/>
----

The application-level `context-param` setting takes precedence over the`webContainer` server setting.
