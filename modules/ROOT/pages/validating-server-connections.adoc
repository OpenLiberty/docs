...
By examining the output from the `https://localhost:9443/ibm/api/config/connectionFactory/cf1` REST endpoint URL, you can find the `validation` REST endpoint path in the `api` field specified with the `/ibm/api/validation/connectionFactory/cf1` value, as shown in the following example:
[source,json]
----
{
