...
These audit events can be captured in the following different CADF format types to help identify different areas where the configured environment can be improved. Open Liberty also generates audit events in xref:ROOT:json-log-events-list.adoc[JSON format]:
