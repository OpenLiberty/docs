...
When you deploy an application with the operator, the operator watches Open Liberty resources and compares the current state of resources to the desired state of resources that you configured.
...
== Servicability and observability with the operator
You can enable persistence for your application by specifying only the size of storage and where you want the storage to be mounted.
...
If you configure storage for servicabli
...
For more information about observability with the Open Liberty operator, see https://github.com/OpenLiberty/open-liberty-operator/blob/master/doc/observability-deployment-rhocp4.2-4.5.adoc[Observability with the Open Liberty Operator].
