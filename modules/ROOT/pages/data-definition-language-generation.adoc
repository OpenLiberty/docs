...
The xref:built-in-jakarta-data-provider.adoc#choosedb[Choosing the Database] section of the built-in Jakarta Data provider documentation has sample server configurations for each possible `dataStore` configuration possible on a repository interface. 
