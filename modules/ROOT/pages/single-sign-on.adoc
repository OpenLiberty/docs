...
link:https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/d2435927-0999-4c62-8c6d-13ba31a52e1a[Active Directory] is a Microsoft service that is used to manage access to protected resources, such as user credentials, on the server. Active Directory enables SSO through the Active Directory Federation Services that manage authentication for applications. When Active Directory is used as an authentication provider in Open Liberty, users are redirected from the Open Liberty server to the Active Directory server to authenticate. When authentication succeeds, the Active Directory server redirects back to the Open Liberty server, where the user gains authorization and is granted access to the application.
