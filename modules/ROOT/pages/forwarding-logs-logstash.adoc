...
[role,command]
----
d:\wlp\bin\securityUtility createSSLCertificate --server=server1 --password="Liberty" --subject=CN=myHostname,OU=defaultServer,O=ibm,C=us
----
...
Created SSL certificate for server s1. The certificate is created with CN=myHostname,OU=server1,O=ibm,C=us as the SubjectDN.
