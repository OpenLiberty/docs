...
If your log data is managed by an external agent, you do not need to use the Logstash collector feature. For more information, see xref:analyzing-logs-elk.adoc[Analyzing logs with the Elastic stack].
...
Set up https://www.elastic.co/logstash[Logstash], https://www.elastic.co/elasticsearch/[Elasticsearch], and https://www.elastic.co/kibana[Kibana] by following https://www.elastic.co[the instructions from Elastic].
...
.. In the `liberty_logstash.conf` file, customize the Beats `ssl_certificate` and `ssl_key` paths with the certificate and key pair files you generated in step 1. Customize the `Elasticsearch_host_name:port_number` Elasticsearch hosts value with the hostname and port number from your Elasticsearch configuration. Customize the Elasticsearch template path with the `liberty_logstash_template_ibm.json` index template file.
...
.. /docs/ref/feature/#logstashCollector-1.0.html[Configure the Logstash collector feature] in your `server.xml` file.

.. Enable https://openliberty.io/docs/ref/general/#access-logging.html[HTTP access logging].
...
== See also
- To see a full list of the available Logstash collector event fields for each log event type, see xref:logstash-event-fields.adoc[Logstash collector event fields].
