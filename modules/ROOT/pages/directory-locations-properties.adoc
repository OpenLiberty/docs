...
For example, the `wlp.install.dir` property can be set in the `server.xml` or `bootstrap.properties` files, depending on when you want the variable to be processed.
For more information about these files and the order in which variables are processed, see the xref:reference:config/server-configuration-overview.adoc[Server configuration overview].
...
For more information about the open source Ant plug-in for Open Liberty, see the https://github.com/OpenLiberty/ci.ant#readme[GitHub repository].
