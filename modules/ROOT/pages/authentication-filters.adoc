...
For example, you can configure an authentication filter for the xref:reference:feature/socialLogin-1.0.adoc[Social Media Login] feature to secure requests from a web application with Google credentials. In the following example, request URLs that contain the `/mywebapp` pattern are authenticated by using Google credentials:

include::reference:pages$feature/socialLogin-1.0/examples.adoc[tag=authfilter]
