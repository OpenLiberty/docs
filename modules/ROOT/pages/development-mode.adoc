...
With the https://marketplace.visualstudio.com/items?itemName=Open-Liberty.liberty-dev-vscode-ext[Open Liberty Tools for VS Code extension], you can start dev mode, make dynamic code changes, run tests, and debug your application, all without leaving the VS Code editor.
...
You also can run commands through the Liberty Dev Dashboard instead of using the command line. For more information, see the https://github.com/OpenLiberty/open-liberty-tools-vscode#commands[full list of commands].
...
After you install the extension and enable either the Maven or Gradle plug-in, your project name is listed under the Liberty Dev Dashboard. You can interact with dev mode by right-clicking on your project name, or selecting a command from the drop-down list for the project name. Alternatively, you can use the **Run Liberty Dev Task** option in the dashboard toolbar.

You can run commands through the Liberty Dev Dashboard instead of using the command line. For more information, see the https://github.com/OpenLiberty/open-liberty-tools-intellij#commands[full list of commands].
