...
In some cases, the configuration that was specified in the bindings and extensions files as a child element must be specified as an attribute in the `server.xml` file. If the child element has only one possible attribute value, such as `shared-session-context` in the application extensions file or `context-root` in the web extensions file, the child element name in the extensions file becomes the attribute name in `server.xml` file. The value of the child element's single attribute becomes the attribute value in `server.xml`.
