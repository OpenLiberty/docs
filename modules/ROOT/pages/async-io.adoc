...
In synchronous read, the application must occupy a thread for the entire time that it takes to receive the data that the client sends.
...
Assume that each inbound request takes 0.1 seconds to process, so the inbound request occupies a thread for 0.2 seconds.
...
If it returns a `false` value, all of the currently available data was read, so the thread is relinquished.
If the `isFinished()` method returns a `true` value, all data was read.
...
If the `onDataAvailable` method returns when the `isReady` method returns a `true` value and all of the data wasn't read, your application must call the `onDataAvailable` method again to restart the read.
...
If the response data isn't available before the `WriteListener` object is registered, the method must return when the `isReady` method is a value of `true`.
