...
|feature:validation-3.1[display=Validation]
...
Consider these definitions when you install Open Liberty and its available features. Each installation option provides the Open Liberty kernel and features that support the platform or Web Profile. Alternatively, you can install individual Jakarta EE features to your existing Open Liberty kernel installation.

Also, consider these definitions when you deploy applications to an Open Liberty server. If your application needs support for enterprise connectivity APIs, you must add platform features to your installation or deploy the application to an Open Liberty installation that has platform features.
...
- xref:reference:jakarta-ee-11-spec-api-maven-coordinates.adoc[Jakarta EE 11 — Complete Specification API Maven Coordinates]
