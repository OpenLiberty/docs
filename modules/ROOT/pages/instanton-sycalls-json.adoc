...
The `criuRequiredSysCalls.json` file grants access to all the Linux system calls that are required by CRIU in order to successfully restore an InstantOn application process. You can assign this file to specify the policy for the container by using the Podman or Docker command options.
