...
The `criuRequiredSysCalls.json` file grants access to all the Linux system calls required by CRIU in order to successfully restore an InstantOn application process. For example, by using the Podman or Docker option `--security-opt seccomp=criuRequiredSysCalls.json`.
